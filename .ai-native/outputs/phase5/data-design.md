...
| `improvement_request_comments` | 記録系（追記のみ。0063） | `id`／`request_id`（紐づく生要望）／`member_id`／`member_name`（スナップショット）／`body`（最大 2,000 字）／`archived_at`（取消）／`created_at` | 生要望の選別（採用/不採用）に伴う **やり取りを時系列で 1 件ずつ**記録（F-42-15）。追加は管理権限者 + 投稿者本人・一覧閲覧は管理権限者のみ（投稿者向け閲覧導線は残課題）。取消は論理削除で復元可（原則9.5）。FK は張らず参照整合は API 書込パスで担保 |
