...
| `Note` | id, memberId, kind(`poipoi`=本人のみ/`minutes`=全員), title, body, projectId?, companyId?, workCategoryId?, source(`text`/`upload`)（バッチ7c = `notes` 0023。**記録系 = 追記のみ**。取込原本は `note_files`。検索インデックスへ自動反映 = poipoi は owner スコープ） | poipoi=C3 / minutes=C2 |
| `SearchDoc` | sourceKind(`company`/`contact`/`industry`/`knowledge`/`project`) × sourceId(一意), title, aliases, body(AI 最適化平文), segments[(entity, field) チェック付き表示単位], bodyHash, embedding(Vertex 埋め込み。無効環境は null), ownerMemberId(null=全員。poipoi は本人のみ = 0023)（バッチ7a = `search_docs` 0021。**派生キャッシュで SoT は各マスタ/ナレッジ本体** = 常に全再生成可能。更新経路 = マスタ書込後の自動再生成（デバウンス）+ 起動時 + `POST /v1/search/reindex` の手動回復（原則6）。照合は生データ・描画は segments の表示項目権限チェック通過行のみ = F-16 準拠） | C2 |
