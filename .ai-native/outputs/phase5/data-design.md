...
| key | text | 設定キー（`^[a-zA-Z][a-zA-Z0-9_.-]{0,63}$`。`currentSegmentId` / `currentChannelId` / **`dashboardLayout`（ダッシュボードの表示・配置。2026-08-03）** / **`dashboardNotificationPlacement`（通知の配置 = `top`/`side`/`bottom`/`hidden` 文字列。'' = 解除。レイアウト本体と別キー = 2026-08-18）** / **`dashboardSectionFavorites`（セクション構成のお気に入り `SectionFavorite[]`。上限 10 件 + 合計 3,800 バイト（純関数で保存前検証 = value 4KB 上限の手前で案内）。2026-08-18）**） |
