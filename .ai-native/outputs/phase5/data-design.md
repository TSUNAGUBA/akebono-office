...
| `WorkflowRoute` | id, category(稟議区分), minAmount, maxAmount, steps[`ApprovalRouteStep`（**stepKind = 承認/決裁/確認・任意。未設定は表示上 最終=決裁・他=承認。approverType に `applicant`〔申請者本人〕を追加 = 提出時に member へ解決して凍結。改善要望 2026-08-17**）], active | C1 |
...
| `PermissionRule` | id, subjectKind(`role`/`title`/`member`), subjectId, resource(機能キー or マスタエンティティ), field?(null=機能全体の利用可否。**フィールドリソースでは「マスタ全体」= 全項目の一括既定（バッチ7m）**/値あり=表示項目), effect(`allow`/`deny`), active（F-16。解決順 個人>役職>ロール・同一レイヤ deny 優先・**同一レイヤ内は明示キー → 一括キー（field=null・member:*）→ の順で参照（バッチ7m）**・どのレイヤにも無ければアプリ既定値（機能・表示項目・日報参照 = allow / AIアシスタント参照 = deny）・既存ロールガードを緩めない制限レイヤ。**field='ai-scope' は AI 参照範囲の擬似フィールド（バッチ7g）: allow = すべて / deny = 自分の登録データのみ。既定は shared AI_SCOPE_FEATURES の区分ごとに定義（poipoi = all / attendance・ai-assistant = own）**。**resource='reports' + field='member:<対象メンバー id>' は日報・週報の参照対象の擬似フィールド（バッチ7h = F-16-6。2026-07-22 で全員の週報にも適用）: deny = その対象者の日報・週報を参照不可。未設定 = 参照可・自分は常に参照可（shared canViewMemberReports）**。**resource='attendance' + field='timecard-all' は全員のタイムカードの参照の擬似フィールド（2026-07-22）: 既定 = 管理者/人事のみ参照可（shared canViewAllTimecards・timecardAllDefault）。allow で一般へ付与・deny で人事から剥奪できる**。**resource='customer-log' + field='member:<対象メンバー id>' は旧「顧客ログの参照対象」の擬似フィールド（2026-07-30 → 改修依頼 2026-08-18 で廃止）: 顧客活動の一覧が全メンバー閲覧可へ変わったため撤去（canViewMemberCustomerLog も削除）。既存ルールは migration 0066 で論理無効化（active=false = 設定履歴の監査可能性を保持）。resource='customer-log' + field=null の機能利用可否は従来どおり有効**。**field='member:*' は参照対象（日報 = reports / AIアシスタント = ai-assistant）の全メンバー一括既定（バッチ7m。'*' はメンバー id として発番されない予約値）**。**field='tab:<タブキー>' はタブ利用可否の擬似フィールド（改修依頼 2026-08-18 = F-16-8）: 既定 allow・機能全体（field=null）へフォールバック。カタログ = shared TAB_PERMISSION_CATALOG**。**field='<項目キー>:write' は項目の更新可否の擬似フィールド（同 = F-16-9）: 参照＞更新の階層 = 参照 deny の項目は :write allow でも更新不可。既定 allow。':write' は camelCase 物理キーと衝突しない予約サフィックス**。**field='ai-scope:member:<id>' / 'ai-scope:title:<役職>' / 'ai-scope:role:<ロール>' / 'ai-scope:*' は登録者単位の AI 参照対象の擬似フィールド（同 = F-16-10）: resource = データ種類（poipoi/attendance/ai-assistant/reports = shared AI_REFERENCE_DATATYPES）・解決順は 個人指定 > 役職指定 > ロール指定 > 一括 > レガシー二値 'ai-scope' > 権限表の参照権限由来の既定（canAiReferenceOwner）。本人の登録データは常に参照可**） | C2 |
| `SystemService` | id, name, description, url, components[{id,name}]（バッチ6c で API 化 = `system_services` 0018。マスタ初期値は home シードと同一の 3 サービスを migration 投入） | C1 |
...
| `MonthlyReport` | id, memberId, monthStart(対象月の初日 YYYY-MM-01), goalReview(今月の成果・達成感), mainWork(今月の主要業務), issues(課題・原因仮説), nextWeek(来月の最重要テーマ = 週報と共通の本文キー・列 next_week を共用), goodPoints?, teamShareKind?（WEEKLY_TEAM_SHARE_KINDS 共通）, teamShareNote?, status（`monthly_reports` 0072 = 改修依頼 2026-08-19 第4弾。**週報と同型で新設**。unique(member_id, month_start)。提出保護は週報と同じ AKO-REP-002） | C3 |
| `ReportComment` | id, reportId, memberId, body, reactions[{memberId, emoji}] | C3 |
| `ReportRead` | memberId(閲覧者)×reportKind(`daily`/`weekly`/`monthly`〔月報 = 改修依頼 2026-08-19 第4弾・0072 で CHECK 拡張〕)×reportId が PK, readAt（`report_reads` 0047 = オペレーター指示 2026-07-31。**全員の日報/全員の週報の既読/未読可視化の SoT**。閲覧状態データ = 記録系の業務データではない。readAt は初回既読時刻を保持（再読で更新しない = ON CONFLICT DO NOTHING・原則2）。「未読に戻す」= 行の物理削除（取消フロー = 原則9.5。監査ログ不要と判断 = 閲覧状態のため）。report_id は daily/weekly の 2 親を kind で切替参照するため FK なし = API 層が対象の存在・参照可否（日報 = コメントスレッドと同じガード / 週報 = 提出済み + F-16-6）を検証してから INSERT） | C3（本人の閲覧状態のみ） |
...
| `Notification` | id, memberId, kind(`approval`/`comment`/`reminder`/`ai_report`/`system`/`escalation`/**`poipoi`（改善のタネ原文の通知。migration 0049 = 2026-08-03）**), title, body, link, read, at | C2 |
...
| `AuditLog` | id, actorId, action, entity, entityId, detail, at（**設定 > 監査ログタブで閲覧可 = 改修 2026-08-21。操作日時/ユーザー/対象ページ/対象データ/操作内容の表示・フィルタ語彙（action/entity → ラベル・ページ）の SoT = `shared/domain/audit-log` の静的カタログ〔両モード共通 = 原則6。モックの camelCase entity は `normalizeAuditEntity` で正規化〕。API 側は `audit_logs` に at DESC / actor_id インデックス = 0081。記録系 = 追記のみ・取消フローなし〔原則9.5 対象外〕**） | C3 |
...
| `WorkCategory` | id, name, displayOrder, active（バッチ7c = `work_categories` 0023。改善のタネ・議事録の任意分類） | C1 |
...
| `CustomerLog` | id, memberId(記録者=所有者), logDate(何月何日=必須), logTime?(開始時刻 HH:MM=任意・**分は 15 分単位から選択 = UI 制約。API は HH:MM を許容 = 旧データ互換**), endTime?(終了時刻=任意・開始がある場合のみ・開始より後 = 0047), companyId(顧客会社=必須), contactId?(顧客担当者=任意・会社に属する), staffMemberId(自社の担当者 = Member 参照・**既定 = ログインユーザー（記録者）**。既存行は member_id でバックフィル = 0047), tags(活動目的 = 旧「属性タグ」〔改修依頼 2026-08-18 で表示名のみ改称・列名 tags は下位互換で維持 = 原則7〕。商談/取材/イベント等。プリセット `CUSTOMER_LOG_TAG_PRESETS` + 自由入力・最大 10 件・各 30cp・重複除去 = 0047), method(活動手段 = 訪問/Web会議/電話/メール/チャット/その他の単一選択・任意・'' = 未設定。allowlist は shared `CUSTOMER_LOG_METHOD_PRESETS` をアプリ層で検証 = 0066・改修依頼 2026-08-18), title?, body(担当者メモ = **旧「会話内容」を意味変更なしで引き継ぐ列**・必須。**議事録メモ〔minutes_memo〕は 0047 で追加後 0050〔2026-08-03〕で廃止 = オペレーター指示**), active(取消=論理削除・復元=取消の取消 = 原則9.5), createdAt/updatedAt（`customer_logs` 0044 → 項目拡張 0047 = オペレーター指示 2026-07-31。**記録系 = 追記 + 本人編集（監査ログ）+ 論理削除/復元**。本人所有・編集/取消は本人のみ。**一覧の表示範囲は改修依頼 2026-08-18 で全員閲覧化 = 全メンバーの有効な記録を認証済み全員が参照できる（GET ?scope=all。旧・許可制の参照権限は廃止 = migration 0066 で既存ルールを論理無効化。他人の取消済みは晒さない）**。AI 参照は本人スコープ = search_docs へ owner 付きで自動反映 = 他メンバーの記録は AI 文脈へ供給しない安全側の既定（全員閲覧化後も維持 = 設計判断）。**会社/担当者はコンボボックス自由入力に対応: newCompanyName/newContactName が未登録名なら同一トランザクションで companies/contacts へ新規登録して反映（正規化名 = shared name-match の完全一致で既存へ名寄せ = 重複マスタを作らない。監査ログあり）**） | C2（顧客とのやり取り） |
| `SupportActivity` | id, memberId(記録者=表示用), receivedDate(受付日=必須), receivedTime?(受付時刻 HH:MM=任意), companyId(顧客会社=必須・コンボボックス新規登録は customer_logs と同一規則 = lib/company-resolve), inquirerName(問い合わせ者=自由入力・任意), targetSystem(対象システム=任意), category(問い合わせ種別 = 操作/不具合/設定/データ/要望/その他), title(件名=必須), body(問い合わせ内容=必須), priority(低/通常/高/緊急), status(未対応/対応中/顧客確認待ち/保留/解決), staffMemberId(自社担当者・既定=ログインユーザー), response/cause/resolution(対応内容/原因/解決内容=任意), completedDate?+completedTime?(完了日時=任意・時刻は完了日必須), knowledgeNote(改善・ナレッジのタネ=任意), active, createdAt/updatedAt（`support_activities` 0067 = 改修依頼 2026-08-18・F-43。**チーム共有の記録系 = 全員が閲覧・登録・編集可（訂正履歴は監査ログ）**。取消=論理削除+復元=原則9.5。区分値の allowlist は shared/domain/types のプリセットをアプリ層で検証。AI 検索インデックスへは供給しない = 安全側。**改修依頼 2026-08-19 第4弾・0071: villageId(事業区分 Village 参照・任意)・firstContactMethod(最初の問い合わせ手段 = AI問合せ/メール/チャット/電話/Web会議/訪問/その他・任意)・targetLocation(対象箇所・自由入力・任意)・links(参考リンク URL 配列 jsonb) を追加。問い合わせ内容は既定テンプレ〔内容/状況/対応〕を投入**） | C2（取引データ） |
| `SalesActivity` | id, memberId(記録者=表示用), companyId(顧客会社=必須・コンボボックス新規登録対応), title(商談名=必須), dealType(新規/追加/更新), staffMemberId(担当者・既定=ログインユーザー), phase(初回/ヒアリング/提案/見積/稟議/受注/失注), amount?(商談金額 bigint 円・任意・0 以上の整数・上限 1 兆), probability?(受注確度 %・任意・0〜100 の整数), expectedCloseDate?(受注予定日), customerIssue/proposal(顧客課題/提案概要=任意), nextAction+nextActionDate?(Next Action/日=任意), active, createdAt/updatedAt（`sales_activities` 0067 = 改修依頼 2026-08-18・F-44。チーム共有の記録系 = SupportActivity と同じ扱い。**改修依頼 2026-08-19 第4弾・0071: villageId(事業区分 Village 参照・任意)・contactId(担当 = 顧客人参照・任意・選択会社に所属)・approachGroup(自由入力・任意)・links(参考リンク URL 配列 jsonb) を追加**。**改修依頼 2026-08-20・0074: 「案件ヘッダー + 活動ログ」構造へ再編 — 既存行 = 案件ヘッダーとして再解釈（データパッチ不要 = 原則7）・aiDigest(jsonb = 活動ログの AI 集約 {summary, highlights?, generatedAt, logCount, llm}。生成→保管→再生成で上書きの導出キャッシュ) を追加。活動ログは `sales_activity_logs`（下記 ActivityLog）へ**） | C2（取引データ） |
| `PartnerActivity` | id, memberId(記録者=表示用), partnerName(パートナー=必須・**自由入力 = 個人名も多くマスタ参照にしない設計判断**), theme(テーマ名=必須), relatedCompany(関連企業=任意・自由入力), activityType(紹介/共創/案件支援/情報交換/その他), status(情報収集/検討/接続予定/進行中/案件化/完了), summary/currentState(概要/現在状況=任意), nextAction+nextActionDate?(Next Action/日=任意), staffMemberId(自社担当者・既定=ログインユーザー), relatedMeeting(関連MTG=任意), relatedSalesActivityId?(関連商談 = sales_activities への任意リンク・FK で実在担保 = 「案件化したら商談へリンク」), memo(任意), active, createdAt/updatedAt（`partner_activities` 0067 = 改修依頼 2026-08-18・F-45。チーム共有の記録系 = SupportActivity と同じ扱い。**改修依頼 2026-08-19 第4弾・0071: villageId(事業区分 Village 参照・任意)・partnerCompanyId(パートナー会社 = 顧客会社参照・必須)・partnerContactId(パートナー担当 = 顧客人参照・任意・選択会社に所属)・approachCompanyId(アプローチ企業 = 顧客会社参照・任意)・approachGroup(自由入力・任意)・links(参考リンク URL 配列 jsonb) を追加し、パートナー/関連企業を自由入力から顧客(会社/人)マスタ参照へ移行。partnerName/related_company は下位互換のため保持し、会社名スナップショット（新規行）・旧行の自由入力（FK 未設定行）の表示に用いる = 原則7**。**改修依頼 2026-08-20・0074: 案件ヘッダー化（SalesActivity と同型）+ initiatives(取組内容・任意) + aiDigest(jsonb) を追加。「概要」（summary）は表示ラベルのみ「背景・目的」へ変更（列名は不変 = 原則7）。活動ログは `partner_activity_logs` へ**） | C2（取引データ） |
| `ActivityLog` | id, activityId(親案件 = sales_activities / partner_activities 参照・FK), memberId+memberName(記録者スナップショット), loggedOn(活動日=必須), kind(訪問/電話/Web会議/メール/その他 = ACTIVITY_LOG_KINDS), title(件名=必須), body(内容), nextAction+nextActionDate?, links(jsonb), active, createdAt/updatedAt（`sales_activity_logs`/`partner_activity_logs` 0074 = 改修依頼 2026-08-20。**チーム共有の記録系 = 全員が閲覧・登録・編集可・取消=論理削除+復元（原則9.5）**。一覧は活動日降順・20件/ページのサーバーページング。検証 SoT = shared/domain/activity の activityLogError。AI 集約（heuristicActivityDigest = 決定的・時系列昇順走査。API は LLM→フォールバック）の材料） | C2（取引データ） |
| `SearchDoc` | sourceKind(`company`/`contact`/`industry`/`knowledge`/`project`/`note`/`document`/`customer-log`/**`manual`（アプリ仕様・運用/操作マニュアル = shared/domain/kb のナレッジベース。0082・改修依頼 2026-08-21。SoT = TS データモジュール〔コードと同期してデプロイで更新〕・権限チェックなし = 全員参照可の公開マニュアル）**) × sourceId(一意), title, aliases, body(AI 最適化平文), segments[(entity, field) チェック付き表示単位], bodyHash, embedding(Vertex 埋め込み。無効環境は null), ownerMemberId(null=全員。poipoi・顧客活動（旧: 顧客ログ）は本人のみ = 0023/0044), links jsonb(`{companyId?, projectId?}` = ノート・顧客活動の紐付け。顧客未指定でも PJ 経由で顧客を補完。質問（今回の質問 → 履歴の新しい順の優先で解決）に別顧客/別 PJ の言及がある場合に AI 文脈から除外 = 混入防止 0024・バッチ7d)（バッチ7a = `search_docs` 0021。**派生キャッシュで SoT は各マスタ/ナレッジ本体** = 常に全再生成可能。更新経路 = マスタ書込後の自動再生成（デバウンス）+ 起動時 + `POST /v1/search/reindex` の手動回復（原則6）。照合は生データ・描画は segments の表示項目権限チェック通過行のみ = F-16 準拠。**チャットボットは使い方・操作系の質問で manual ブロック（`app_manual` ツール / kbFindRelated）を参照して回答（RAG）。モックは shared/domain/kb を直接照合 = 同一ロジック**） | C2（manual は C1 = 公開マニュアル） |
...
| `HearingLog` | id, memberId, date, kind(`qa`=ヒアリング回答/`memo`=改善のタネ), calendarEventId, question, answer, at | 記録系（追記のみ・巻き戻し禁止） | C3（課題回答を含み `DailyReport` と同水準） |
...
| `AppConfigItem` | key, value（例: reportInputMode = `form`/`assist`/`both`。**バッチ7h 追加キー: `teamVisibleMemberIds` = チームタブ表示メンバーの JSON 配列（''/空 = 既定表示 = マトリクスは社員・契約・アルバイト / タイムラインは全員。バッチ7k で候補 = 在籍中の全メンバー）・`menu-categories-dashboard` / `menu-categories-masters` = メニューカテゴリ定義の JSON（'' = 既定構成）・**`dashboard-layout` = ダッシュボードの表示・配置（`DashboardLayout` の JSON。テナント全社既定。'' = 未設定 → `menu-categories-dashboard` 下位互換 or デフォルト。F-13-9・2026-08-03）・**`dashboard-notification-placement` = 通知の配置（`top`/`side`/`bottom`/`hidden` の文字列。'' = 未設定。解決は層整合 = ユーザー配置キー > ユーザー層レイアウト由来 > 本キー > レイアウト由来（既存ユーザーレイアウトの配置選択を上書きしない = 原則7）。**レイアウト本体と別キー = 配置変更でセクション構成を固定しない**。2026-08-18）・`poipoi-notify-recipients` = 改善のタネ通知の宛先（`NotifyRecipientTarget[]` の JSON。ロール/役職/個人。'' = 通知しない。F-12-5・F-13-10・2026-08-03）**） | 設定系（upsert 更新可。SoT は本アプリ） | C1 |
...
| `ProductSku`（product_skus） | id, productId, code, janCode, axis1/2Value, sellPrice/costPrice（null = 商品既定）, isDefault, active, **custom jsonb（F-31・0042 で追加。一覧列は今回スコープ外）** | 設定系。マトリクス生成は既存軸値の組をスキップ = 冪等・生成で既定 SKU 無効化 | C2 |
...
| `PurchaseOrder`（purchase_orders） | id, code, companyId, segmentId, status（状態機械 = shared PO_STATUS_NEXT）, orderDate, dueDate, **lines jsonb**, note, **custom jsonb（項目カスタマイズ F-31）** | 指示系（遷移のみ・DELETE なし） | C2 |
| `ProductionOrder`（production_orders） | id, code, skuId, qty, warehouseId, dueDate, status, **results jsonb（追記のみ）**, **custom jsonb（F-31）** | 指示系 + 実績（実績登録 = 追記 + 在庫 production_in + 全数完成で completed。1 トランザクション） | C2 |
...
| `InboundResult`（inbound_results） | id, code, planId, warehouseId, receivedAt, lines jsonb（planLineId 消込）, **custom jsonb（F-31）** | **記録系（追記のみ）**。登録 = 実績 + 在庫 inbound(+) + 予定ステータスを 1 トランザクション | C2 |
| `PurchaseRecord`（purchase_records） | id, code, companyId, segmentId, purchaseDate, purchaseType, warehouseId（入荷管理 OFF 経路の入庫先）, lines jsonb（**GIN `jsonb_path_ops` = 0034**）, correctionOf, **custom jsonb（F-31）** | **記録系（訂正は赤黒）**。warehouseId ありは purchase_in(+)・訂正で在庫も戻す。二重訂正は 409。委託精算の原価解決は `lines @> [{"skuId":…}]` で対象 SKU の最新仕入 1 件を取る（GIN が支える = Codex P1-1） | C2 |
...
| key | text | 設定キー（`^[a-zA-Z][a-zA-Z0-9_.-]{0,63}$`。`currentSegmentId` / `currentChannelId` / **`dashboardLayout`（ダッシュボードの表示・配置。2026-08-03）** / **`dashboardNotificationPlacement`（通知の配置 = `top`/`side`/`bottom`/`hidden` 文字列。'' = 解除。レイアウト本体と別キー = 2026-08-18）** / **`dashboardSectionFavorites`（セクション構成のお気に入り `SectionFavorite[]`。上限 10 件 + 合計 3,800 バイト（純関数で保存前検証 = value 4KB 上限の手前で案内）。2026-08-18）** / **`notificationChannels`（通知種別×通知先マトリクス `NotificationMatrix`。既定セル〔in_app=ON・外部=OFF〕は保存しない最小形 = serializeNotificationMatrix。2026-08-20）**） |
...
| テーブル | 区分 | 主な列 | 備考 |
|---|---|---|---|
| `improvement_requests` | 記録系（追記のみ・**SoT**） | `id`／`member_id`／`member_name`（スナップショット）／`page_path`・`page_label`（投稿元 or 選択した対象。**空パス + ラベル「全体」「新設ページ」も可 = 2026-08-17**）／**`target_spot`（対象箇所 = ページ内のどこか。自由入力・任意・既定 ''。0069・改修依頼 2026-08-19 第4弾 → 受付箱の一覧表示・詳細編集 = 2026-08-21）**／`body`／**`status`（受付箱ステータス = 改修依頼 2026-08-21・0080 で CHECK 差替: 新規書込は基底 6 値 `unconfirmed`（未確認 = 起票時の初期値）/`reviewing`（検討中）/`planned`（改善対応 = AI 集約対象）/`operational`（運用対応）/`deferred`（継続検討）/`dismissed`（対応見送り）。旧値 `open`/`resolved` は保存値として受理し `improvementInboxStatusOf` が読み替える〔保存値は書き換えない = 原則7〕。表示の「対応済み（addressed）」は集約先 item の進捗からの導出・「解決済み」は `resolved_at` オーバーレイ = どちらも保存しない〔原則6〕。既定 unconfirmed）**／**`revisit_on`（date・継続検討の再検討日 = deferred への遷移で必須）・`revisit_notified_on`（date・リマインド済マーカー = 冪等制御。部分インデックス improvement_requests_revisit_idx。0080）**／**`resolved_at`（timestamptz・解決済みの記録 = 基底 status を上書きしないオーバーレイ。取消で NULL へ戻せる = 原則9.5。0080）**／**`ai_assessment`（jsonb・AI 判定 `{verdict, reason, docIds, llm, assessedAt}` = ナレッジベース照合の導出キャッシュ。再判定で上書き・NULL = 未判定。0082）**／**`links`（jsonb・添付 URL 配列。0061）**／**`images`（jsonb・添付画像 `{filename,mime,dataUrl}` 配列。data URI は商品画像と同じ allowlist + 上限 400,000 字。0061）**／**`adoption`（旧・選別列。2026-08-21 のステータス一本化で新規書込を停止 = 保存済みの値は `improvementInboxStatusOf` が読み替える〔pending → 未確認・adopted → 改善対応・declined → 対応見送り = 原則7〕。0063）**／**`tags`（jsonb・任意タグ配列。`brainstorm`〔壁打ち〕/`entrust`〔お任せ〕の allowlist = shared `normalizeImprovementTags`。既定 `[]` = 旧データ互換〔原則7〕。0065・2026-08-18）**／**`excluded_item_ids`（jsonb 配列・既定 `[]`。「集約の解除」で外した改修単位 id の履歴 = 次回以降の AI 集約でそこへは再追記しない〔往復 + detail 重複・「対象外」メモとの矛盾防止〕。履歴は蓄積 = クリアしない〔二重解除でも過去の単位へ戻らない〕。0065・F-42-19）**／`item_id`（集約先・NULL=未集約）／`archived_at`（取消）／**`edited_at`（timestamptz・NULL=未編集。本文の最終編集時刻 = 「編集済み」の明示。0064・2026-08-18）**／`created_at` | 投稿は認証済み全員可。取消は論理削除（`archived_at`）で復元可（原則9.5）。**投稿者本人または管理権限者が編集可（`POST /requests/:id/edit`。改修依頼 2026-08-19 で本文だけでなく tags/links/images も編集可 = 登録時の全項目。shared `improvementRequestEditFields` で検証・部分更新〔実在キーのみ更新 = 未指定は保持。CLAUDE.md 部分更新の鉄則〕。取消済みは不可 = 先に復元。編集は上書きだが edited_at で明示し再編集で戻せる = 原則9.5）**。添付の参照はリンク = 別タブ・画像 = 押下で拡大（F-42-11）。**改修プロンプトでは tags は出力しない〔人間運用用〕・受付箱のコメントは反映する〔改修依頼 2026-08-19〕**。ステータスはプロンプト再生成に反映（F-42-12。【解決済み】= 再改修しない・【対応見送り】= 実装しない）。0063 で既存の集約済み行は `adoption='adopted'` へバックフィル（下位互換 = 原則7）。**受付箱の状態機械・解決フロー・AI 判定のエンドポイントは api-design の `/requests/:id/status`・`/resolve`・`/assess` を参照（2026-08-21）** |
| `improvement_items` | 導出（人手のステータス・編集を持つ） | `id`／`title`／`summary`／`detail`／`status`（**3 状態へ再編 = 改修依頼 2026-08-21・0080 で CHECK 差替: 新規書込は `todo`（未対応 = 集約直後の初期値・既定）/`in_progress`（対応中）/`done`（対応済）のみ。旧 7 値語彙（`triage`/`accepted`/`operational`/`deferred`/`resolved`/`rejected` = 〜0073）は保存値として受理し `improvementItemViewOf` が 3 状態へ読み替える〔triage/accepted/deferred → 未対応・resolved/operational/rejected → 対応済。保存値は書き換えない = 原則7〕。運用対応・継続検討の一次フローは受付箱（improvement_requests）へ移管**）／**`assignee_member_id`・`assignee_name`（担当者 = 対応中への遷移時にアサイン可・後からでも変更/解除可〔原則9.5〕。名前はスナップショット。一覧・カンバン・ガントに表示。0080）**／**`revisit_on`・`revisit_notified_on`（date。旧 deferred 案件〔〜2026-08-20〕の再検討日リマインド用に残置 = 期日どおり届け続ける〔原則7〕。新規の deferred は要望側）**／`page_paths`（jsonb）／`source_request_ids`（jsonb）／`llm`／`archived_at`／`created_at`・`updated_at`／`resolved_at`（done で記録・reopen でクリア）／**`plan_start`・`plan_end`（date・任意 = 対応予定期間。0058）** | AI 集約で生成。**再集約は未集約要望のみ処理し判定済み item を巻き戻さない（原則2）**。取消/復元可。予定期間はガントチャートで可視化。done への遷移で紐づく要望の起票者へ通知（受付箱の表示は「対応済み」へ導出） |
...
| `improvement_request_comments` | 記録系（追記のみ。0063） | `id`／`request_id`（紐づく生要望）／`member_id`／`member_name`（スナップショット）／`body`（最大 2,000 字）／**`kind`（`ops` = 運用対応遷移の自動記録〔運用案内〕/ `comment`〔既定〕= 手動コメント。0083・R3 監査 2026-08-21 = 本人開示の判別キー）**／`archived_at`（取消）／`created_at` | 対応方針検討（旧・選別のやり取り含む = 原則7）に伴う **やり取りを時系列で 1 件ずつ**記録（F-42-15）。追加は管理権限者 + 投稿者本人・一覧閲覧は管理権限者のみ。**例外: 投稿者本人は「自分の要望 + requestId 指定」で運用案内（kind='ops'）のみ取得可（R2 監査 2026-08-21 = 通知 OFF でも案内を読める到達経路。一般コメントの本人向け閲覧は引き続き未提供 = 意図的な情報開示方針）**。取消は論理削除で復元可（原則9.5）。FK は張らず参照整合は API 書込パスで担保。**有効コメントは改修プロンプト（`buildCodingPrompt`）に各要望のサブ項目として反映する（改修依頼 2026-08-19）** |
