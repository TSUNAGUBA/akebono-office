...
| テーブル | 区分 | 主な列 | 備考 |
|---|---|---|---|
| `improvement_requests` | 記録系（追記のみ・**SoT**） | `id`／`member_id`／`member_name`（スナップショット）／`page_path`・`page_label`（投稿元）／`body`／**`links`（jsonb・添付 URL 配列。0061）**／**`images`（jsonb・添付画像 `{filename,mime,dataUrl}` 配列。data URI は商品画像と同じ allowlist + 上限 400,000 字。0061）**／`item_id`（集約先・NULL=未集約）／`archived_at`（取消）／`created_at` | 投稿は認証済み全員可。取消は論理削除（`archived_at`）で復元可（原則9.5）。添付の参照はリンク = 別タブ・画像 = 押下で拡大（F-42-11） |
