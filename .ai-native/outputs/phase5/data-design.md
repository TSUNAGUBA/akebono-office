...
| テーブル | 区分 | 主な列 | 備考 |
|---|---|---|---|
| `improvement_requests` | 記録系（追記のみ・**SoT**） | `id`／`member_id`／`member_name`（スナップショット）／`page_path`・`page_label`（投稿元 or 選択した対象。**空パス + ラベル「全体」「新設ページ」も可 = 2026-08-17**）／`body`／**`status`（`open`/`resolved`/`dismissed`・既定 open。要望単位の対応状況 = 改修単位ステータスとは独立の進捗タグ。0062）**／**`links`（jsonb・添付 URL 配列。0061）**／**`images`（jsonb・添付画像 `{filename,mime,dataUrl}` 配列。data URI は商品画像と同じ allowlist + 上限 400,000 字。0061）**／**`adoption`（`pending`/`adopted`/`declined`・既定 pending。生要望の選別 = 採用のみ AI 集約対象。集約済み行は変更不可・0063）**／**`tags`（jsonb・任意タグ配列。`brainstorm`〔壁打ち〕/`entrust`〔お任せ〕の allowlist = shared `normalizeImprovementTags`。既定 `[]` = 旧データ互換〔原則7〕。0065・2026-08-18）**／**`excluded_item_ids`（jsonb 配列・既定 `[]`。「集約の解除」で外した改修単位 id の履歴 = 次回以降の AI 集約でそこへは再追記しない〔往復 + detail 重複・「対象外」メモとの矛盾防止〕。履歴は蓄積 = クリアしない〔二重解除でも過去の単位へ戻らない〕。0065・F-42-19）**／`item_id`（集約先・NULL=未集約）／`archived_at`（取消）／**`edited_at`（timestamptz・NULL=未編集。本文の最終編集時刻 = 「編集済み」の明示。0064・2026-08-18）**／`created_at` | 投稿は認証済み全員可。取消は論理削除（`archived_at`）で復元可（原則9.5）。**投稿者本人または管理権限者が編集可（`POST /requests/:id/edit`。改修依頼 2026-08-19 で本文だけでなく tags/links/images も編集可 = 登録時の全項目。shared `improvementRequestEditFields` で検証・部分更新〔実在キーのみ更新 = 未指定は保持。CLAUDE.md 部分更新の鉄則〕。取消済みは不可 = 先に復元。編集は上書きだが edited_at で明示し再編集で戻せる = 原則9.5）**。添付の参照はリンク = 別タブ・画像 = 押下で拡大（F-42-11）。**改修プロンプトでは tags は出力しない〔人間運用用〕・受付箱のコメントは反映する〔改修依頼 2026-08-19〕**。ステータスはプロンプト再生成に反映（F-42-12）。0063 で既存の集約済み行は `adoption='adopted'` へバックフィル（下位互換 = 原則7） |
| `improvement_items` | 導出（人手のステータス・編集を持つ） | `id`／`title`／`summary`／`detail`／`status`（`triage`/`accepted`/**`in_progress`（対応中 = 改修依頼 2026-08-18・0068 で CHECK 差替。値の SoT = shared/domain/improvement.ts）**/**`operational`（運用対応）/`deferred`（継続検討）= 改修依頼 2026-08-20・0073 で CHECK 再差替。「対応方針」語彙へ再編 = accepted の表示は「改善対応」・rejected は「対応見送り」（保存値は不変 = 原則7）**/`resolved`/`rejected`）／**`revisit_on`（date・`deferred` で必須 = 再検討日。0073）・`revisit_notified_on`（date・再検討日通知の済マーカー = /jobs/improvement-revisit-reminders の冪等制御。0073）**／`page_paths`（jsonb）／`source_request_ids`（jsonb）／`llm`／`archived_at`／`created_at`・`updated_at`／`resolved_at`／**`plan_start`・`plan_end`（date・任意 = 対応予定期間。0058）** | AI 集約で生成。**再集約は未集約要望のみ処理し判定済み item を巻き戻さない（原則2）**。取消/復元可。予定期間はガントチャートで可視化 |
