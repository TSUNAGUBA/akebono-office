...
| `CustomerLog` | id, memberId(記録者=所有者), logDate(何月何日=必須), logTime?(HH:MM=任意), companyId(顧客会社=必須), contactId?(顧客担当者=任意・会社に属する), title?, body(会話内容=必須), active(取消=論理削除・復元=取消の取消 = 原則9.5), createdAt/updatedAt（`customer_logs` 0044。**記録系 = 追記 + 本人編集（監査ログ）+ 論理削除/復元**。本人所有・本人のみ操作可。他メンバーの参照は権限表 `customer-log` + field `member:<id>` の allow = 許可制（既定 = 参照不可・canViewMemberCustomerLog）。AI 参照は本人スコープ = search_docs へ owner 付きで自動反映 = 他メンバーのログは AI 文脈へ供給しない安全側の既定） | C2（顧客とのやり取り） |
...
| エンティティ（テーブル） | 主要属性 | 分類 / SoT | 機密度 |
|---|---|---|---|
| `ImportSource`（import_sources） | id, name, method（file_csv/file_fixed/file_json/api_pull）, encoding（utf8/sjis）, targetEntity, schedule, active, **config jsonb**（0043。方式別設定 = CSV: hasHeader/delimiter・API: endpoint/authType/authValue・JSON/API: jsonRootPath。method 別に使う項目のみ保持） | 設定系（CRUD・論理削除で取消/復元 = 原則9.5）。SoT = 本テーブル。config は PUT /:id/config で更新（正規化 = shared normalizeImportSourceConfig）。**config.authValue（秘匿値）は GET で管理者にのみ実値・非管理者はマスク = C3 相当の最小権限** | C2（authValue は C3） |
| `ImportMapping`（import_mappings） | id, sourceId, version, status（draft/active/superseded）, **fields jsonb**（sourceField/targetItemKey/transform ＋ 方式別ロケータ = **columnIndex**〔CSV 列番号〕/ **byteStart·byteEnd**〔固定長 1 始まり両端含む〕/ **jsonKey**〔JSON/API〕。0043）, createdAt。**UNIQUE(source_id, version)** + 部分一意 `(source_id) WHERE status='active'` | 設定系/版管理（新版を追記し旧 active は superseded = 上書きせず履歴を残す）。version 採番は advisory lock で直列化 | C2 |
