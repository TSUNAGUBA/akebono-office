...
| `useNotes` | ぽいぽいメモ・議事録（デュアルモード: API = `/v1/notes` が SoT・検索インデックス自動反映 / モック = notes コレクション。バッチ7c/7d）。テキスト登録 + ファイル取込（モックは .md/.txt のみ）+ 取消（archive = 論理削除。poipoi = 本人 / minutes = 登録者 or 管理者） |
