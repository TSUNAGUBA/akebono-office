...
**業態アプリのトップ配置 + 入力既定の集約（2026-07-28 増分・オペレーター指示）:** 業態ごとの Akebono 業務を「アプリ」としてダッシュボードに直接並べ、押下でその業態の業務へ入場する（`/akebono?seg=<id>` → 現在業態を切替。従来の「単一 AKEBONO カード → ヘッダで業態切替」を置き換え。ヘッダスイッチャは入場後の切替手段として残す = 非破壊）。各業態アプリは**表示名（`appName`。未設定は業態名）とアイコン（`appIconImage` = アップロード画像を優先、無ければ `appIcon` = 選択式 lucide キー、無ければ業種タイプ既定 `INDUSTRY_DEFAULT_ICON`）**で直感識別する（`segmentAppName`/`segmentIconKey`。共有部品 `AkebonoSegmentIcon`。※ダッシュボード最上段の固定表示部品 `AkebonoSegmentApps` は改善要望 2026-08-21 で廃止 = 業態カードは `useAkebonoAppCards` によるセクション配置へ統合。screen-design『ダッシュボード』参照）。設定は**業態アプリ設定**画面（`/akebono/settings/segments`。共通マスタ管理配下・管理者ゲート）に集約する。あわせて**商品登録の入力コスト最小化**として、`事業セグメント`（= 入場した業態で固定）・`単位`・`課金区分`・`バリアント軸1/2ラベル`の既定値を業態設定に持たせ、商品の通常登録・編集フォームからは外して自動適用する（`segmentFieldDefaults`）。個別に上書きしたい場合のみ商品側の**「カスタマイズ」フォーム**（別セクション）でこの 5 項目を編集する。「業態の既定に戻す」で取消可能（原則 9.5）。追加属性はすべて任意で未設定時はフォールバックするため下位互換（原則 7）。
...
- 取消（F-32-5): マスタ系 = 反映時に旧値 JSON を ImportRun に保全 → 復元（**取込後に更新された行は復元スキップ + 警告** = updated_at ガードで lost update を防ぐ）。トランザクション系 = 赤黒一括生成。**決定 #9（2026-07-20）で確定**
  - **実装状況（2026-07-30 = implementation-status §48-1）:** 実取込の初期実装では旧値保全・一括取消は**未実装**。是正は各アプリの既存フロー（商品/取引先 = 編集・無効化 / 売上 = 赤黒訂正 / 在庫 = adjust）で個別に行う。決定 #9 の一括取消・原本保全（決定 #14）は後続スコープとして残課題に記録
...
> **実装済みの値域（SoT = `shared/domain/import-run.ts` の IMPORT_TRANSFORMS。2026-08-07 ② で UI 選択式化）**: `''`（変換なし = 前後空白のみ除去）/ `number`（¥・カンマ除去）/ `date`（YYYY-MM-DD へ統一。時刻付き日時「YYYY/MM/DD HH:MM:SS」・ISO も日付部分のみへ。2026-08-10）/ `upper` / `lower`。上記構想の残り（zenhan/fixedValue/codeLookup/concat/split 等）は未実装。外部コード → マスタ ID の突合は codeLookup ではなく**マスタ間連携キー（lookupField）**として実装済み。
...
| 取込 | `GET/POST /v1/akebono/import-sources`・`PUT /import-sources/:id/config`（方式別設定）・`/import-sources/:id/archive`・`/restore`・`GET/POST /v1/akebono/import-mappings`（版管理）・`GET/POST /v1/akebono/import-runs`（実行履歴） | 実装済み（F-32 Phase D + Part② + 実取込 = implementation-status §48-1）。取込元は方式別マッピング編集で左辺=取込元・右辺=対象アプリ有効項目。実行は実取込（ファイル添付 / SSRF ガード付き API pull → マッピング適用 → 対象テーブルへ反映。段階 API・非同期化・一括取消は後続） |
...
| 8 | 委託精算のマージン率の定義（店舗取り分 or 当社取り分）が未確定（決定 #5 の付帯 = 「設定で柔軟に」） | **モックアップは実運用に整合する「店舗取り分」で実装**（当社の店舗宛請求 = 売上 × (1 − 店舗取り分率)。店舗取り分 + 作家支払 + 当社粗利 = 売上）。受け入れ基準に「店舗請求 − 作家支払 ≥ 0」を追加。**2026-08-10 = オペレーター確認済み**（委託仲介の運用検証で「上代を販売店・弊社・作家の三者へ按分」= 店舗取り分モデルで確定）。改修 P1 で**弊社取り分（残余 = 1 − 店舗取り分 − 作家率）を可視化**（締め前プレビュー）し、店舗取り分 + 作家率 > 100% の逆ざや設定を非ブロッキング警告（settlementIntegrity）= 整合を設定段階で担保 |

## 11. 受け入れ基準（Phase 5 ゲートへ向けて）

> **モックアップ実装状況（2026-07-20）:** v1 全域（12 ページ + 13 composable）を `home/` に実装。typecheck / build / 単体テスト（委託精算の三者整合・税・在庫畳込・採番のリグレッションテスト含む）green。独立ロールレビュー 4 観点 + 修正の再レビューを実施し、BLOCKER/MAJOR の未解決指摘ゼロ。

- [x] discussion-points の全論点がオペレーター判断済み（第 1 巡 14 論点 + 第 2 巡 #5 付帯確認 3 点・2026-07-20 完了。**未決ゼロ**）
- [x] サイトマップ・画面定義（12 ページ）実装済み。I/F 6 視点はモックアップで検証（在庫台帳導出・請求確定系・委託精算の設定注入純関数）
- [x] データ設計が正規化原則・データ 3 分類・SoT 宣言（§3.6）を満たす（在庫 = 台帳 SoT・売上 = 明細 SoT・請求 = 発行後不変）
- [x] モックアップがダミーデータ（陶磁器 + SI/SaaS + アパレル SKU）で UC-09〜13 を一気通貫で操作できる（商品→入荷→在庫→出荷→店舗預け→売上→請求/委託精算）
- [x] 全データフロー（伝票 → 在庫 → 売上 → 請求）が I/F レベルで矛盾なく通る（冪等キー・赤黒訂正・発行後不変を含む）
- [x] **委託精算の整合: 店舗請求 − 作家支払 = 当社粗利 ≥ 0**（銀座例: 10,080 − 8,640 = 1,440 = 売上の 10%）
- [x] **壁打ち残（実装後の確認）: 委託精算のマージン率の定義（店舗取り分 or 当社取り分）** = **2026-08-10 オペレーター確認済み**（委託仲介の運用検証で「店舗取り分」モデルと確定）。改修 P1 で弊社取り分の可視化・逆ざや設定の整合検証（settlementIntegrity / settlementPreview）を追加（リスク #8 解消）
