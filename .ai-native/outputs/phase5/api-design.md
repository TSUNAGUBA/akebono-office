...
| 目標マスタ（goals = F-01 コックピット着地予報。2026-07-29） | `GET/POST/PATCH /v1/masters/goals`（+ `/:id/archive` `/:id/restore`。汎用マスタ CRUD へ追加 = registry + migration 0039・id プレフィックス `g`。`metric='segment_sales'` は segmentId 必須 / `report_rate` は全社（segmentId null）・値 0-100 = **POST は create スキーマの superRefine・PATCH は masters.ts の goalCrossGuard（既存行とマージした結果で検証 = workflow-routes と同型のクロスガード方式）+ goals テーブルの CHECK 制約（0039）で担保**（.partial() 由来の patchSchema はクロスフィールド検証を引き継がないため）。**一覧 GET はサーバー側行フィルタ（C2）**: admin / sales 許可ユーザー = 全件・hr（sales deny 時）= report_rate 全件 + 自分の担当業態の segment_sales（pr-def-05 と整合。レビュー2巡目 G2）・それ以外は自分の担当業態（members.segment_ids）の segment_sales のみ（report_rate は返さない）。**members へ segmentIds（`segment_ids` jsonb・既定 `[]`）を追加** = 部分 PATCH は既存の hasOwn フィルタで未送信キーを保持（Zod v4 注意の回帰テストに segmentIds を追加済み）） |
...
| useAkebonoImports（データ取込 F-32 = Phase D） | `GET /v1/akebono/import-sources`・`/import-mappings`・`/import-runs`（参照は全員）・`POST /v1/akebono/import-sources`（**管理者のみ**。AKO-IMP-001）+ `/:id/archive` `/:id/restore`（論理削除で取消/復元 = 原則9.5）・`POST /v1/akebono/import-mappings`（**版管理** = 新版を追記し旧 active は superseded・version 採番は advisory lock で直列化・部分一意 INDEX が二重 active を防ぐ。項目 0 行は AKO-IMP-003）・`POST /v1/akebono/import-runs`（**記録系・追記のみ**。有効マッピングなしは AKO-IMP-002 409・実行はサーバーが決定的にシミュレートして履歴を記録 = 隔離行 AKO-IMP-010。実ファイル取込は F-32 後続）（**実装・フロント接続済み = Phase D**。機能ガード 'akebono'。デュアルモード = モックモードは従来同期・挙動不変） |
