...
| note 連携（F-40 拡張・2026-08-12。**非公式 API** = 下書きまで・公開は人間が note 管理画面で） | **連携**: `GET /v1/media/note/status?channelId=`（connected/urlname/hasCookie/cookieStorable。Cookie 値は返さない）・`PUT /v1/media/note/connection`（管理者。urlname + 任意の sessionCookie = `_note_session_v5`。部分更新 = hasOwn・Cookie は AES-256-GCM 暗号化 = 036、チャンネル未存在は 021）・`POST /v1/media/note/disconnect`（管理者。原稿・スナップショットは残す = 非破壊）。**原稿**: `GET/POST /v1/media/note/posts?channelId=`（fromGeneratedId で AI 生成記事から取込）・`PATCH /:id`（title/body/noteKey = 手動公開記事の紐付け）・`POST /:id/archive` `/:id/restore`（原則9.5）・`POST /:id/push`（管理者。マークダウン→note HTML・**下書き送信のみ**・再プッシュは同一下書き更新 = 冪等・公開済みは 409。Cookie 未設定/失効 = 034・note 失敗 = 033）。**収集**: `POST /v1/media/note/sync`（公開エンドポイントから当日スナップショット upsert = 冪等 + 公開検知 = 一方向）・`GET /v1/media/note/metrics?channelId=&days=`（期間内デルタ集計）・ジョブ `POST /jobs/note-sync`（CRON_SECRET。deploy-guide §1-7d）。**フィードバック**: `POST /v1/media/note/feedback/generate`（Vertex AI → heuristicNoteFeedback フォールバック。media_insights scope='note' へ upsert）・参照は既存 `GET /v1/media/insights?scope=note`・**記事生成の fromInsightId が scope='note' も受ける**（nextTopics もヒント化）= 「反応 → 次の記事」の循環（implementation-status §76） |
...
| AKO-MEDIA-030 | note 連携が未設定（同期・プッシュに urlname 登録が必要。409） | ✅ |
| AKO-MEDIA-031 | note 連携の入力不正（urlname 形式・Cookie 長・noteKey 形式） | ✅ |
| AKO-MEDIA-032 | 対象の note 投稿（記事原稿）が見つからない | ✅ |
| AKO-MEDIA-033 | note API 呼び出し失敗（非公式 API = 仕様変更の可能性を明示。note 応答の抜粋付き。502） | ✅ |
| AKO-MEDIA-034 | note セッション Cookie が未設定・復号不可・失効（下書きプッシュに必要。再登録を案内） | ✅ |
| AKO-MEDIA-035 | note 投稿のタイトル・本文未入力（作成・プッシュ） | ✅ |
| AKO-MEDIA-036 | note Cookie を保存できない（TOKEN_ENCRYPTION_KEY 未設定。409） | ✅ |
