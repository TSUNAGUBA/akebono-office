...
| 目標マスタ（goals = F-01 コックピット着地予報。2026-07-29） | `GET/POST/PATCH /v1/masters/goals`（+ `/:id/archive` `/:id/restore`。汎用マスタ CRUD へ追加 = registry + migration 0035・id プレフィックス `g`。`metric='segment_sales'` は segmentId 必須 / `report_rate` は全社（segmentId null）・値 0-100 = create/patch 両スキーマの superRefine で検証。**members へ segmentIds（`segment_ids` jsonb・既定 `[]`）を追加** = 部分 PATCH は既存の hasOwn フィルタで未送信キーを保持（Zod v4 注意の回帰テストに segmentIds を追加済み）） |
