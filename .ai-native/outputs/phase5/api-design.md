...
| `GET /v1/improvements/request-comments` | **生要望コメントの一覧（`requestId`／`includeArchived`〔管理のみ〕。古い順。0063）。非管理者は「自分の要望」+ `requestId` 指定に限り、運用案内（「運用案内: 」接頭辞 = 起票者へ通知した本文と同一の記録）だけを取得できる（R2 監査 2026-08-21: 運用案内の本人到達を system 通知だけに依存させない = 通知 OFF でも要望ドロワーで読める。管理検討のコメントは従来どおり管理のみ）** | 管理（運用案内のみ = 投稿者本人も可） |
