...
| useCustomerContext（顧客コンテキスト F-46 = 改修依頼 2026-08-20・**事業メモ = 0084・改修依頼 2026-08-21 第2弾**） | `GET /v1/customer-contexts`（全件ハイドレーション互換の bare 配列。`q` = ビジョン/経営課題/補足メモ/**事業メモ**/会社名の部分一致）・`PUT /v1/customer-contexts/:companyId`（**1社1行 upsert**〔ON CONFLICT company_id〕・**部分更新 = body に実在するキーのみ**〔`Object.hasOwn` = Zod v4 の `.partial()` 既定値注入バグ回避と同型〕・4 項目〔vision/challenges/strategyNotes/**businessNotes**〕すべて空は AKO-CTX-001・会社なし AKO-CTX-002。検証の SoT = shared/domain/customer-context = mock パリティ〔原則6〕）・`GET/POST /v1/customer-contexts/:companyId/notes`（時系列 Memo。limit/offset・`POST /:noteId/archive|restore` = 論理取消/復元〔原則9.5〕・メモなし AKO-CTX-003）・`POST /:companyId/research`（Web 調査 = Vertex 検索グラウンディング generateGroundedText → 失敗時は決定的ヒューリスティック〔原則4〕。候補 = タイトル + URI + 抜粋）・`POST /:companyId/build`（採用ソース 1〜8 件 → generateJson〔BUILD_SCHEMA = **businessNotes 含む 4 項目**〕→ 決定的フォールバック。現在値との差分確認用に current も返す）・`POST /:companyId/apply`（upsert + research ノート自動追記 = **同一トランザクション**。payload = `{ sources, before }` = 反映前の値を保存）・`POST /:companyId/revert`（research ノートの before から復元 + `revertedAt` 追記 = ノートは archive しない監査可能な取消〔原則9.5〕。二重取消 no-op = 冪等・before 欠落 AKO-CTX-004。**旧スナップショット〔before.businessNotes キーなし〕は現在の事業メモを保持** = 原則7）。機能ガード customer-context |
