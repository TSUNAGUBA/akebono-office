...
> **設計 SoT: `cockpit-design.md`**（§3 の画面構成・§3.2 の優先順位表・§3.3 のロール適応が正）。本節はサマリー。

縦順（モバイル 375px = この順で 1 カラム / ③ の KPI は 2 列・lg 以上は 4 列グリッド）:
1. **CockpitStrip**: 挨拶・氏名・日付 + フェーズバッジ（punchState × punchRequired。出勤前/勤務中/休憩中/退勤済み・punch 対象外は非表示）+ 今日の完了メーター（細バー。一手 0 件の日は非表示）+ 着地予報 1 行（タップで 4 を開閉）
2. **CockpitMoves（次の一手）**: 優先順位付きキュー（未退勤打刻 → 出勤/退勤打刻 → 稟議承認 → 勤怠承認 → 日報 → シフト希望 → エスカレーション → 有給5日。cockpit-design §3.2）。最優先 1 件は大カード（punch 系は WidgetsPunchClock flat 埋込）+ 続く手 最大 4 行 + 「ほか N 件」。全消化は ok トーンの完了表示
3. **CockpitInstruments（計器）**: 自分の計器（今月実働/有給残/36協定）・シフト計器（parttime × shift）・労務計器（hr/admin: 承認キュー・36協定該当・年5日未達 = 件数のみで個人名は出さない）・経営計器（sales 権限: 今月売上+前年比）・事業計器（Member.segmentIds 優先・未設定は明示的な業態選択のみ）+ 週次インサイト 1 行（保存済み personal のみ）。**該当データ・権限がない計器は枠ごと非表示**
4. **CockpitForecast（着地予報。既定: 閉）**: ロール別の滑走路バー 1〜4 本（CSS のみ: 実績塗り bg-brand + 予測マーカー + 目標ティック。トーンは ok/warn の 2 値）+ 各行に reason 1 行 + 針路修正 1 件（最大乖離の warn への導線）。goals 未設定は「目標が未設定です」+ admin のみ `/masters/goals` 導線
5. AKEBONO 業務（業態別アプリ。2026-07-28 のセクションを維持）
6. すべての機能 = カード型メニュー（**格下げ配置**。カテゴリチップ・カスタマイズ・sessionStorage 記憶は §5.3 のまま不変）
7. 通知フィード（直近 5 件・タブ構成は従来どおり）

- 打刻はヘッダーの「タイムカード」ボタン → モーダル（PunchClock 再利用 + `/attendance?tab=daily` リンク。モーダル内リンクで遷移すると自動クローズ）も従来どおり併存
...
- 自分の日報: 日付ナビ（上段 = ← / 今日 / →（月ビュー時は ← / 今月 / →）・下段 = 選択中の日付を直接選択可）+ **表示モード「週」「月」トグル（週 = 週ストリップ・月 = 1 日〜末日の横スクロール / カレンダー形式の 2 ビュー。日クリックで選択日切替 = 2026-07-22）** + **共通ヘッダ 1 行のテーブル形式エントリ入力（テーマ自由入力 + 内容 + 時間ステッパー 0.25h + 進捗 = 2026-07-22 で名称変更・テーブル化**・旧データのプロジェクトはテーマ表示へフォールバック）+ 所感/課題 + **明日の予定（テーマ/目的/内容/時間 × 最大 3 件。翌営業日の日報エントリへ自動反映 + 反映元バナー。旧自由記述は互換表示）** + 提出ボタン。時間と勤怠の乖離警告
- 全員の日報（バッチ5e）: 対象月ナビ（← / 今月 / → + 月直接選択 = 「自分」の日付ナビと対称。指示 2026-07-21 #2）+ **部署・メンバー絞り込み（2026-07-22。AI 社員の日報は絞り込み未指定時のみ表示）** + 提出済み日報の一覧（日付・名前・サマリー・時間。UiDataTable = PC 行 / モバイル カードの自動切替）→ 押下で詳細ドロワー（CommentThread 付き）
...
- 全員の週報（F-06-11 = 2026-07-22 新設）: サブタブ（全員の週報 / AI インサイト = 旧週報タブから移設）+ 週ナビ + 部署・メンバー絞り込み + 選択週の提出済み週報一覧（参照可否は権限表「日報・週報の参照対象」F-16-6）→ 押下で詳細ドロワー
