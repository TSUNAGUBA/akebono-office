...
### `/media` メディア分析（F-40。AKEBONO 業務配下・2026-07-28 追加、本実装 = 同日）
- `/media`: ハブ（MediaSegmentBar = 対象業態切替 + GA 連携バッジ → 未連携は MediaGaConnect ゲート / 連携済みはサマリー KPI + 機能カード + 全業態一覧）
- `/media/analytics`: タブ（メディア分析 / 業務 × メディア PDCA）。KPI 8 枚 + 日別/チャネル/デバイス/記事別チャート + セクション表 + AI インサイト（生成 → 保管 → 再生成）。API モードはロード中・取得失敗（再試行導線）・データ空・部分失敗警告を区別表示。PDCA タブは統合取得失敗時に 0 表示せず失敗表示 + 再試行（M1）。売上軸は Phase C（sales_records 移行）で実データ化 = デモデータバッジは撤去
- `/media/articles`: AI 記事生成スタジオ（フォーム → プレビュー（品質スコア・Vertex AI/自動生成表示）→ 採用（セクション選択）/ 取消・復元。「分析からお題を提案」）
- `/media/settings`（管理者）: 業態セレクタ + GA 連携（モック = 擬似同意モーダル / API = OAuth リダイレクト → 復帰 ?ga= クエリ → GA4 プロパティ選択モーダル。needsProperty の中間状態から再開可）+ AI 分析設定フォーム
