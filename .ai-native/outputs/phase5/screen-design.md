...
- フィルタ: 検索（会社名・担当者・内容）+ 顧客（会社）絞り込み
- 一覧: 日時（時刻は任意）・顧客会社・担当者・件名・冒頭。押下で詳細モーダル（全文 + 記録者 + 編集履歴。自分の記録は編集/取消ボタン）。編集/取消アイコンは自分の記録のみ
- 登録・編集モーダル: 日付（必須）・時刻（任意）・顧客会社（必須）・担当者（任意・選択会社に属する連絡先のみ）・件名（任意）・会話内容（必須）
- 取消済み（自分のみ）: 折りたたみ表示 + 「元に戻す」で復元（原則9.5）
- AI 参照: 登録した記録は AI（チャットボット・日報アシスト）の参照対象（**自分の記録のみ** = 本人スコープ）
- モバイル: 一覧はカード型・モーダルはフルワイド
