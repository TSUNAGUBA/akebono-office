...
- `/media/analytics`: タブ（メディア分析 / 業務 × メディア PDCA）。KPI 8 枚 + 日別/チャネル/デバイス/記事別チャート + セクション表 + AI インサイト（生成 → 保管 → 再生成）。API モードはロード中・取得失敗（再試行導線）・データ空・部分失敗警告を区別表示。PDCA タブは GA 月次失敗時に 0 表示せず失敗表示 + 再試行（M1）・売上軸のデモデータ明示バッジ（salesRecords 移行まで）
