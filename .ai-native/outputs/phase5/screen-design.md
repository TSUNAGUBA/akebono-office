...
ナビゲーション:
- **PC**: サイドメニューなし（2026-07-16 オペレーター指示で廃止）。ヘッダー（ロゴ=ホームリンク・ページタイトル・ホームボタン・通知ベル・デモユーザー切替）+ 遷移はダッシュボードのカード型メニュー起点。未定義ルートは 404 ページ（pages/[...slug].vue）で受ける
...
- 自分（AI アシスト入力。F-06-7/8）: 設定が `both` のときは「通常入力 / AI アシスト」切替トグルを表示。アシスト時は ①Google カレンダー連携ゲート（未連携: 擬似 OAuth 同意モーダル / 連携済み: 状態バー+解除）②スケジュール & タスク（Google 同期・タスク追加・Google へ反映。タスク追加は未連携でも可）③ぽいぽいメモ ④AI ヒアリング（チップ+自由記述、答え直し可）⑤ドラフト生成 → 生成根拠バナー付きでフォームに流し込み、確認・修正して提出。提出済みの日は再生成不可 + メモ/ヒアリング入力を閉じる
