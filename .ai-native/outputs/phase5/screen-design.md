...
- 全員の日報（バッチ5e）: 対象月ナビ（← / 今月 / → + 月直接選択 = 「自分」の日付ナビと対称。指示 2026-07-21 #2）+ 提出済み日報の一覧（日付・名前・サマリー・工数。UiDataTable = PC 行 / モバイル カードの自動切替）→ 押下で詳細ドロワー（CommentThread 付き）
...
- チーム: 週ナビ（← / 今週 / → + 週レンジ表示。指示 2026-07-21 #2）+ 提出状況マトリクス（メンバー×その週の営業日: ✓/未/休）+ リマインド送信（本日以前の最新営業日が対象）+ 日報詳細ドロワー（CommentThread 付き）
- 週報: 週ナビ（← / 今週 / → + 週レンジ表示。指示 2026-07-21 #2）で参照週を選択し、その週の自分の週報を編集・提出
