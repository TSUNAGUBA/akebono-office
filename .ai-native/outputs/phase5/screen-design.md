...
- 自分の日報: 日付ナビ（上段 = ← / 今日 / →（月ビュー時は ← / 今月 / →）・下段 = 選択中の日付を直接選択可）+ **表示モード「週」「月」トグル（週 = 週ストリップ・月 = 1 日〜末日の横スクロール / カレンダー形式の 2 ビュー。日クリックで選択日切替 = 2026-07-22）** + **共通ヘッダ 1 行のテーブル形式エントリ入力（テーマ自由入力 + 内容 + 時間ステッパー 0.25h = 2026-07-22 で名称変更・テーブル化**・**「進捗」列は入力フォームでは非表示 = オペレーター指示 2026-07-31。既存データの進捗値は編集・保存でも保持し、参照表示（提出済み表示・詳細ドロワー）には引き続き出る = 原則7**・旧データのプロジェクトはテーマ表示へフォールバック）+ 所感/課題 + **明日の予定（テーマ/目的/内容/時間 × 最大 3 件。翌営業日の日報エントリへ自動反映 + 反映元バナー。旧自由記述は互換表示）** + **ぽいぽいポスト欄（フォーム最下部 = オペレーター指示 2026-07-31。入力があると日報の保存・提出と同時にトップメニューのぽいぽいポストと同一経路（useNotes 'poipoi'）で登録・空欄はスキップ。登録成功で欄をクリア = 再保存でも二重登録しない・登録失敗は警告のみで日報フローは止めない = 原則4。日付/ユーザー切替でクリア）** + 提出ボタン。時間と勤怠の乖離警告
...
- 自分の週報: 週ナビ（← / 今週 / → + 週レンジ表示。指示 2026-07-21 #2）で参照週を選択し、その週の自分の週報を編集・提出
