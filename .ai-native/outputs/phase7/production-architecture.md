...
- **未移行ドメイン**（シフト・AI アシスタント・AI カンパニー等のフロント）は API モードでもモックデータで動く
...
6. **バッチ3b（本 PR）:** ワークフロー・稟議 API + フロント接続（申請・経路凍結・承認・代理・証跡・承認経路マスタ）— 完了
7. **バッチ3 続き:** シフト → AI業務アシスタント（LLM API キー要）→ カレンダー連携（OAuth クライアント要。LLM/OAuth はサーバーサイド）
8. **バッチ4:** 意思決定支援・AI カンパニー・売上・稼働状況・チャットボット・mart ETL
