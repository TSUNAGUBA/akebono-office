...
- **ドメイン別キャッシュ（バッチ2b-1 で確立）:** 通知・日報のように期間・キー単位で遅延ロードするドメインは、
  モジュールスコープの API キャッシュを `useMockDb.tbl()` の代わりに射影ロジックへ差し込む（画面・射影は共通のまま）。
  ログイン確立・切替時は `onApiReset` フックで取り直す。通知は 60 秒ポーリングで新着を反映
- **未移行ドメイン**（勤怠・休暇・エスカレーション等のフロント）は API モードでもモックデータで動く
  （バッチ2b-2 以降で順次接続。移行状況の SoT は implementation-status.md）
...
3. **バッチ2b-1（本 PR）:** 通知 + 業務日報のフロント接続（inbox・ダッシュボード・コメント/リアクション・リマインド）— 完了
4. **バッチ2b-2:** 勤怠・休暇のフロント接続（タイムカード・日次/週次/月次・36 協定・休暇管理）
5. **バッチ3:** AI業務アシスタント・カレンダー連携・ワークフロー・シフト・エスカレーション API（LLM/OAuth はサーバーサイド）
6. **バッチ4:** 意思決定支援・AI カンパニー・売上・稼働状況・チャットボット・mart ETL
