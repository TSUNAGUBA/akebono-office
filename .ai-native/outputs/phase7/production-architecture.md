...
- **権限ルール（F-16・バッチ5c）:** ロール/役職/個人の 3 レイヤの権限ルール（`permission_rules`）で機能利用・表示項目をさらに制限できる（解決順 = 個人 > 役職 > ロール・未設定は許可）。ロールガードの**上に重なる制限レイヤ**であり、ロールガードを緩めることはできない。判定は shared/domain/permissions.ts をフロント/API で共有
...
15. **バッチ5c（本 PR）:** 権限制御基盤 F-16（3 レイヤの権限ルール・機能ガード・表示項目レベル制御・権限設定 UI）
16. **バッチ5 続き:** チャットボットの全 DB 参照化（権限準拠）
17. **バッチ4 続き（バッチ5 の後）:** AI カンパニー・売上 + mart ETL・稼働状況
