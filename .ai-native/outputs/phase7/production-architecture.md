...
- **未移行ドメイン**（エスカレーション・ワークフロー・AI アシスタント等のフロント）は API モードでもモックデータで動く
  （バッチ3 以降で順次接続。移行状況の SoT は implementation-status.md）
...
4. **バッチ2b-2（本 PR）:** 勤怠・休暇のフロント接続（タイムカード・日次/週次/月次・36 協定・打刻修正・休暇管理）— 完了
