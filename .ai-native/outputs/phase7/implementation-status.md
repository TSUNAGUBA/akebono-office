...
- [x] **① 修正**: UiMultiCombobox に開閉方向の自動判定を追加 — 開くたびに実測し、下の残り空間が候補リスト高（max-h-56 + マージン = 240px）未満かつ上の方が広ければ**上方向（bottom-full）に開く**。汎用対応のため権限設定等の他の利用箇所でも同様に効く（下に収まる配置では従来どおり下開き。デスクトップでも下端付近では上開きになる = 意図した挙動）
