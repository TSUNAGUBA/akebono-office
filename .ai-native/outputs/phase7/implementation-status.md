...
- [x] **shared/domain/import-run.ts（applyImportTransform）**: `date` 変換の正規表現に**時刻部の切り落とし**を追加。
  `^(\d{4})[/.\-]?(\d{1,2})[/.\-]?(\d{1,2})(?:[T ].*)?$` とし、日付部の後に**空白 or `T` で始まる残り**（時刻・タイムゾーン）が
  続く値を日付部のみへ正規化する。例: 「2026/08/09 19:40:48」「2026-08-09T19:40:48+09:00」→「2026-08-09」。
  日付部を含まない値（例: 「19:40:48」）・不正形式は従来どおり素通し = 呼び出し側の `DATE_RE`（akebono-imports.ts）で隔離。
- [x] **IMPORT_TRANSFORMS カタログの hint と docstring** を更新（時刻付き＝日時は日付部分だけを取り出す旨・発生日の列にも使える旨）。
  カタログの value 集合（`'' / number / date / upper / lower`）は不変 = UI 選択肢・対応集合テストは追加なしで維持（原則5）。
- **設計判断**: 隔離メッセージの案内どおり「列に `date` 変換を指定すれば通る」を成立させる層（＝変換）で解決する。
  `DATE_RE` 自体は緩めない（変換未指定で日時を暗黙受理すると、誤った列マッピングを検知できなくなるため = 選択式変換の設計と一貫）。

### 63-2 実装：棚卸確定のマイナス実棚数を許容
...
### 63-3 スコープ外（記録）：F-32 在庫取込の `qty` 意味論
- 在庫**取込（F-32・applyInventory）**の `qty` は対象項目カタログ上 **「増減数（デルタ）」**（item-settings ラベル・
  `Math.abs(qty)` で負値許容済み）であり、棚卸確定エンドポイントのような「実棚数→理論差分」の自動計算は**行わない**（デルタ追記）。
  本報告②の記述「実物理在庫数が取込元」は**棚卸確定（/inventory/stocktake）**に対応するため本節で充足。もし F-32 取込側でも
  「実物理在庫数を絶対値として受け、理論差分を計上」する意味論が必要なら別スコープの改修（対象項目の追加 or kind 別解釈）として扱う。

### 63-4 検証（実測値）
- [x] api typecheck green・mockup typecheck green。
- [x] api unit：`applyImportTransform` に日時ケース（`2026/08/09 19:40:48`・ISO・区切り差・時刻のみ素通し）を追加。
- [x] api 統合：棚卸テストに**マイナス実棚数**（wh-01 実棚 −3〔理論 6〕→ −9 調整・残高 −3）を追加。

### 63-5 下位互換・データ影響（原則7）
- [x] スキーマ変更なし。`date` 変換は**より多くの入力を受理する方向**の拡張（従来通る値の結果は不変・従来素通しだった日時が正しく変換される）。
  棚卸は**受理域を負に広げる**のみ（既存の 0〜100 万の挙動は不変）。既存マッピング・既存台帳データへの遡及影響なし。
