...
- **バッチ3d（本 PR）:** AI業務アシスタント（F-14）+ 日報 AI アシスト（F-06-7）の API 化 + フロント接続（LLM 失敗時はモックと同一ヒューリスティックへフォールバック）
- **バッチ3 続き:** カレンダー連携（OAuth クライアント要）。その後バッチ4（AI カンパニー・売上・稼働状況・意思決定・チャットボット応答・mart ETL）
...
| AI アシスト材料サマリ・ドラフト生成 | F-06-7 | ✅ | 🚧 `/v1/assist`（logs・answers・memos・report-draft）| 🚧 | 本 PR。ドラフト = Vertex AI 構造化出力 + 出力正規化 → 失敗時 shared ヒューリスティック。生成結果は保存しない。カレンダー材料（F-06-8）は連携バッチで拡張 |
...
| AI業務アシスタント `/ai-assistant`（計画・AI コメント・振り返り・日報反映・インサイト） | F-14 | ✅ | 🚧 AI アシスタント接続（本 PR）: task_plans/assist_logs（0008）・計画 CRUD（本人のみ・結果記録後は不変）・AI コメント（Vertex AI → ヒューリスティック）・結果記録（FOR UPDATE で 1 回確定）・管理者インサイト（サーバー集計）・日報ドラフト生成 | カレンダー予定からの計画化・材料はカレンダー連携バッチで拡張 |
