...
- [x] AI 参照範囲の権限化（F-16-5）: `PermissionRule` の擬似フィールド `ai-scope`（allow = すべて / deny = 自分の登録データのみ。スキーマ不変 = 既存レイヤ解決を再利用）+ shared `aiReferenceScope()`。対象区分 = AI_SCOPE_FEATURES（ぽいぽいポスト・勤怠・タスク計画/カレンダー = 本人スコープを持つドメイン）。**機能自体の deny が最優先**（機能が使えないユーザーの AI には当該ドメインを供給しない = 従来どおり）
