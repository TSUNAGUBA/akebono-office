...
- **バッチ3a（本 PR）:** エスカレーション（F-12）の API 化 + フロント接続（起票・対応・ナレッジ還流・36協定/日報課題/チャットボットの発火）
- **バッチ3 続き:** ワークフロー → シフト → AI業務アシスタント（LLM API キー要）→ カレンダー連携（OAuth クライアント要）。その後バッチ4（AI カンパニー・売上・稼働状況・意思決定・チャットボット応答・mart ETL）
...
|---|---|:---:|:---:|---|
| ダッシュボード（カードメニュー + 通知） | F-01-1/2 | ✅ | ✅ 通知のフロント接続済み | 通知は 60 秒ポーリング + 画面操作で反映。未接続ドメイン（ワークフロー等）発の通知は、各ドメインの API 化（サーバー発火へ移行）まで API モードでは表示されない |
| 通知・エスカレーションセンター `/inbox` | F-12 | ✅ | 🚧 エスカレーション接続（本 PR）: 起票（日報課題・36協定・チャットボット）→ 対応（回答/裁定/対応不要）→ ナレッジ還流まで API 化 | 通知タブは PR #17 で接続済み。AI カンパニー発シグナル（停滞・過負荷）はバッチ4 |
...
## 3. 今回バッチ（3a: エスカレーション）の完了条件（Definition of Done）

- [x] escalations テーブル（0005）+ `/v1/escalations`（一覧 = 管理者のみ / 起票 = dedupe + クールダウン冪等 / 解決 = open→resolved クレーム + ナレッジ還流 + 本人通知）
- [x] サーバー発火: 日報提出の課題起票（`PUT /v1/reports/daily` が `escalated` を返す）・36協定チェック（`POST /v1/escalations/overtime-check`）
- [x] useEscalations デュアルモード化・inbox エスカレーションタブ接続（モックバッジ除去）・チャットボット起票の実データ接続
- [x] 検証: API 統合テスト 36 / API モード実クリック E2E / モック回帰
- [ ] バッチ3 続き: ワークフロー → シフト → AI アシスタント（LLM キー要）→ カレンダー連携（OAuth 要）
