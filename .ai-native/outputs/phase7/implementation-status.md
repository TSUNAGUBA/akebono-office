...
- **バッチ3c（本 PR）:** シフト表（F-05）の API 化 + フロント接続（募集期間の状態機械・希望提出・割当バリデーション・確定公開・本人合意）+ バッチ3b レビュー指摘対応（下書きの可視性など）
- **バッチ3 続き:** AI業務アシスタント（LLM = Vertex AI に決定・オペレーター指示 2026-07-17。ADC 認証のためキー不要）→ カレンダー連携（OAuth クライアント要）。その後バッチ4（AI カンパニー・売上・稼働状況・意思決定・チャットボット応答・mart ETL）
...
| シフト表 `/shift` | F-05 | ✅ | 🚧 シフト接続（本 PR）: 募集期間の状態機械（正順のみ・FOR UPDATE クレーム）・希望提出（本人・open 中・締切内）・割当（調整中のみ・労基法34/61条/週40h バリデーション共有）・確定公開（割当 confirmed 化 + 通知）・確定後変更の本人合意・必要人数 | 希望・割当の参照は管理者 = 全件 / 本人 = 自分のみのサーバースコープ。デモユーザー切替ボタンはモックモード限定 |
...
## 3. 今回バッチ（3c: シフト表）の完了条件（Definition of Done）

- [x] shift_periods / shift_wishes / shift_assignments / shift_demands テーブル（0007。希望・割当・必要人数は UNIQUE upsert = 設定系の上書き）
- [x] `/v1/shifts`: 一括ハイドレーション GET（希望・割当は管理者 = 全件 / 本人 = 自分のみ）/ 期間（POST /periods + /:id/transition = 正順の状態機械を FOR UPDATE クレームで直列化。published 遷移は割当 confirmed 化 + スタッフ通知まで一括）/ 希望（PUT /wishes + POST /wishes/clear = 本人・open 中・締切内のみ）/ 割当（POST /assignments + /:id/unassign = 調整中のみ、/:id/request-change + /:id/consent = 確定後変更の本人合意 2 段階）/ 必要人数（PUT /demands = 0 人で削除）
- [x] 割当バリデーション（労基法34条 休憩・61条 年少者深夜 = 割当不可・週40h・希望NG）を `shared/domain/shift.ts` へ切り出し、フロントのリアルタイム警告と API ガードで同一ロジックを共有
- [x] useShifts デュアルモード化・shift.vue 接続（確定シフト・希望提出・調整グリッド・募集期間・モックバッジ除去。デモユーザー切替はモック限定化）
- [x] バッチ3b レビュー指摘対応: 他人の下書き稟議の一覧露出を本人・管理者のみに制限 / 提出後通知の非ブロッキング化 / 下書き更新のエラー区分をモックと統一 / 代理人の外部委託ガード / 経路マスタの金額帯・ステップ順序のサーバー検証（PATCH は既存行とマージして検証）
- [x] 反復レビュー（原則9）: 独立レビュー 2 巡目の指摘（割当・解除と確定公開の期間ロック直列化 / 必要人数の小数入力で 500 / メッセージ・日付表示のモード間整合）を反映し、重大・軽微とも未対応ゼロ
- [x] 検証: API 統合テスト 46 / API モード実クリック E2E 7 スイート 78 チェック / モック回帰 25 チェック + シフトモックスモーク 4
- [ ] バッチ3 続き: AI アシスタント（LLM = Vertex AI・基盤は本 PR で導入）→ カレンダー連携（OAuth 要）
