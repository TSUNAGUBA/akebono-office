...
- **バッチ2b-2（本 PR）:** 勤怠・休暇のフロント接続（タイムカード・日次/週次/月次・36 協定・打刻修正・休暇管理）
...
| 36 協定アラート | F-04-4 | ✅ | ✅ `GET /v1/attendance/alerts` | 🚧 | 直近 6 ヶ月の全平均判定。エスカレーション連携はバッチ3（F-12 API 化後） |
...
| 工数乖離チェック（勤怠実労働との差） | F-06-3 | ✅ | ✅ 提出レスポンスの `hoursGapMinutes` | ✅ | 60 分超のみ返却。提出時警告はサーバー計算値。一覧・編集中の乖離表示は本 PR（勤怠接続）で有効化 |
...
|---|---|:---:|:---:|---|
| ダッシュボード（カードメニュー + 通知） | F-01-1/2 | ✅ | ✅ 通知のフロント接続済み | 通知は 60 秒ポーリング + 画面操作で反映。未接続ドメイン（ワークフロー・エスカレーション等）発の通知は、各ドメインの API 化（サーバー発火へ移行）まで API モードでは表示されない |
| 通知・エスカレーションセンター `/inbox` | F-12 | ✅ | ✅ 通知タブ接続済み / エスカレーション API は⏳ | エスカレーションタブはバッチ3（API モードではモックデータのまま） |
...
## 3. 今回バッチ（2b-2）の完了条件（Definition of Done）

- [x] 勤怠のフロント接続（打刻ウィジェット・日次/週次/月次・36 協定・打刻修正申請/承認・管理者タイムカード）
  - 集計はサーバー（/v1/attendance）。月単位のサマリキャッシュから日次・週次を射影（60h 超の月累計を保持）
- [x] 休暇のフロント接続（残数・年 5 日義務・申請/承認・個別/一括付与）
  - grants/requests をハイドレーションし、残数射影（FIFO 引当）は共通ロジック。最終判定はサーバー
- [x] 日報の工数乖離表示を有効化（勤怠接続により編集中・提出済みバッジとも実データで判定）
- [x] API 追補: 打刻修正申請の全件参照を管理者/人事に開放（承認は管理者のみのまま）
- [x] 検証: API 統合テスト（32 ケース）/ mockup 回帰（typecheck・35 tests・generate・モック実クリック 28 チェック）/ **API モード実クリック E2E 37 チェック（打刻→状態遷移→日次反映・打刻修正申請→承認→反映・付与→申請→承認の永続化・タイムカードまで確認）**
