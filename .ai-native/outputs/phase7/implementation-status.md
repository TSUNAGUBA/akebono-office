...
- [x] R2 = R1 修正の独立再検証。コード・テスト・挙動は全項目 PASS（typecheck 復旧・reads の kind 別ガードの
...
- [x] **パーサ/描画の拡張（utils/markdown.ts + UiMarkdown.vue。既存呼び出し元は挙動不変）**:
  ①**パイプテーブル**（GFM 形式 = ヘッダー + 区切り行。LLM の表回答対応。表内横スクロール = 原則8。
  区切り行がなければパラグラフへフェイルオープン = 本文中の | を壊さない）
...
- [x] テスト: markdown.test.ts（テーブル分解・フェイルオープン・linkifyRoutes の最長一致/非破壊/コード除外）+
