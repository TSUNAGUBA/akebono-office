...
改善要望（F-42）に寄せられた 4 要望への対応（対象 = `/improvements` と投稿ウィジェット）。

- [x] **(1) 受付箱のタイムスタンプ秒表示**: 受付箱一覧の「投稿日時」列と要望詳細ドロワーを
  `yyyy/MM/dd HH:mm:ss` 表示にする（`fmtDateTimeSec` を `utils/format.ts` に追加。壁時計のまま = TZ 変換しない・
  形式外はそのまま返すフォールバック。単体テストで固定）。
- [x] **(2) 要望タグ「壁打ち」「お任せ」（F-42-17）**: 投稿フォームに「タグ（任意）」（`UiChipSelect`・複数可）。
  型・ラベル・意味の SoT = shared `IMPROVEMENT_REQUEST_TAG_META`（brainstorm = 壁打ち〔対話での要件整理を経て
...
- [x] **(3) タブ再構成 + 一覧内/一括選別（F-42-18）**: 文言修正 生要望（受付箱）→ **受付箱**。ページ上部に
  ...
  **`UiTabBar`（受付箱/改修案件/カンバン/ガントチャート。受付箱に未選別バッジ・`?tab=` は `useRouteTabSync`）**。
  受付箱の一覧に行内「採用」「不採用」ボタン（同じ選別の再押下 = 未選別へ戻す取消フロー = 原則9.5。
  集約済み・取消済みは操作なし）と複数選択の一括選別（「すべて選択」+「まとめて採用/不採用」。
  `setRequestAdoptionBulk` = 既存 1 件 API の逐次呼び + 反映 1 回。部分成功は done/failed の件数トーストで案内 =
  原則4。絞り込み変更・集約で対象外になった選択は自動解除）。選別操作・選択列はモバイルのカード表示にも出す（原則8）。
- [x] **(4) 画像添付の D&D・貼り付け**: §84 で対応済み（ドロップエリア + ウィンドウ全体のファイルドロップ抑止 +
  Ctrl+V/⌘V 貼り付け。共通 `addImageFiles` = 原則3）を確認・変更なし（本改修単位では受入確認のみ）。
- [x] **(5) 集約の解除（F-42-19・追加指示 2026-08-18）**: 集約済みの要望を改修単位から外し
  「採用済み（集約待ち）」へ明示的に戻す = 再度「AI で集約」の対象にする（`POST /requests/:id/uncluster`・
  `unclusterRequest`）。改修単位のステータス・本文は不変（原則2）・`source_request_ids` からは除去（原則6）。
  ガードは共有 `improvementUnclusterError`（存在 404 → 未集約 AKO-REQ-017 → 取消済み AKO-REQ-018 = 単体テスト固定）。
  導線 = 受付箱ドロワー（集約済み表示）+ 改修単位ドロワーの元要望カード（確認ダイアログ → トースト。
  AKO-REQ-013 の案内文も「集約の解除」に言及するよう更新）。**解除した item は `excluded_item_ids`（0065・履歴 = 蓄積・クリアしない）に
  記録し、次回以降の AI 集約でそれらの item へは再追記しない**（ヒューリスティック・LLM 正規化の両方で除外 =
  解除 → 再集約が同じ単位への往復 + detail 重複にならない。二重解除でも過去の単位へ戻らない = レビュー R1/R6）。
...
- [x] **ドキュメント整合（原則5）**: functional-requirements（F-42-17/18 新設）・screen-design（5.7 タブ構成・
  ...
  プロンプトのタグ加味）・CONVENTIONS（useImprovements / ImprovementSubmit）・本節。

> 補足: 一括選別の専用 API は追加しない（既存 1 件エンドポイントの検証・監査ログを再利用 = 原則3。
> 件数規模は管理画面の手動選択が前提で逐次呼びで十分。将来スケールで必要になればサーバー側バッチを検討）。
>
> R1 = 独立レビュー（高）で指摘 5 件 → 全件反映: ①解除 → 再集約が同じ triage item へ戻り detail が重複
> （往復 no-op + テキスト汚染）→ 解除元の除外（当初 `unclustered_from`・R6 で `excluded_item_ids` の履歴方式へ改称。上記）②サマリーカードのタブ直行が残留検索語と
> 合成され件数と一覧が食い違う → goItems で検索をクリア ③adoption ルートの旧コメント（外すには archive のみ）
> が uncluster 追加後も残留 → 更新 ④一括選別の仕分け・件数算定が未テスト → 共有純関数 `planAdoptionBulk`
> へ抽出し単体テスト固定 ⑤adoption が取消済み行を黙って更新（一括選別の古い選択との競合で選別が復活する）→
> `archived_at IS NULL` ガード + AKO-REQ-019（mock も同一判定・統合テスト追加）。
> R2 = 再レビュー（高）で指摘 5 件 → 全件反映: ①ヒューリスティックの除外がグループ単位で、除外の無い
> 同ページ要望まで新規 item へ巻き添え（LLM 正規化の要望単位除外と乖離）→ 追記先を要望ごとに解決
> （除外者だけ別 item/新規へ）②一括選別（API モード）が全滅時に refresh せずキャッシュ陳腐のまま再試行
> ループ → 常に 1 回 refresh ③api-design のエラーコード表に AKO-REQ-015〜019 が未登載 → 登載
> ④タグの後編集フロー無し（原則9.5）→ 送信直後の取消 → 再送で正せることを明記 + タグ単体編集を残課題化
> ⑤選別ガード（存在/集約済み/取消済み + メッセージ）が mock・shared・API の 3 か所へ分散コピー →
> 共有 `improvementAdoptionError` へ一本化（planAdoptionBulk・mock setRequestAdoption・API ルートが共用）。
> R3 = 再レビュー（高）で指摘 4 件（低〜中）→ 全件反映: ①受付箱カードのタブ直行が残留チップフィルタと
> 食い違う → goInbox で「未選別」へ揃える（goItems と同じ規則）②集約解除に busy ガードが無く二重発火で
> 成功直後に AKO-REQ-017 の誤エラートースト → unclusterBusy + ボタン disabled ③選別可否の条件
> `!itemId && !archivedAt` がテンプレへ 3 か所インライン → 共有ガード準拠の `isTriageable`
> （improvementAdoptionError === null）へ一本化 ④選別変更の競合フォールバックが AKO-REQ-019 を
> 別意味で再利用（復元では直らない）→ 専用コード AKO-REQ-020 を新設・エラーコード表へ登載。
> R4 = 再レビュー（高）で指摘 5 件（低〜中）→ 全件反映: ①解除後も元 item の detail に旧記載が残り、
> プロンプトが解除済み要望を含み続ける → 解除時に「【集約の解除】…実装対象に含めないこと」の修正メモを
> 自動で残す（文言 = shared `buildUnclusterNoteBody`・mock/API 同一。メモは担当者メモとしてプロンプトに載る）
> ②adoption ルートのフォールバック SELECT が archived_at を生 timestamptz（Date）で受けており型宣言と不一致 →
> to_char へ統一 ③select/ops 仮想列に飾りのソートヘッダ → TableColumn.sortable:false を新設し非ソート描画（X-1）
> ④サマリーカードのクリック可能 boilerplate が 5 連コピー → summaryCards 配列 + v-for へ集約
> ⑤受付箱一覧テンプレの `(row as unknown as ImprovementRequest)` 二重キャスト散在 → `reqOf(row)` ヘルパへ集約。
> R5 = 再レビュー（高）で指摘 3 件（**うち 1 件はクラッシュ級**）→ 全件反映: ①R4 の reqOf 置換が
> `関数名(row as unknown as ImprovementRequest)` の呼び出し括弧まで食い、`isTriageablereqOf(row)` 等の
> 未定義識別子へ壊変（受付箱タブが描画時 TypeError・typecheck 失敗。パイプ `| tail` で typecheck の
> 終了コードが握り潰されて検知漏れ → pipefail で再検証する運用に修正）→ 全 12 か所を
> `isTriageable(reqOf(row))` 等へ修正 + 取り残しの二重キャスト 2 か所も reqOf へ ②シードにタグのデモを
> 追加したのに SEED_VERSION 未更新（既存 localStorage が当日中stale）→ v22 へバンプ ③①の取り残しキャスト
> （同上で解消）。
> R6 = 再レビュー（高）で指摘 3 件（低〜中）→ 全件反映: ①解除元の記録が「最後の 1 件」のみで、
> 二重解除（A → B → 解除）後の再集約が「対象外」メモの残る A へ戻り得る → `excluded_item_ids`（jsonb 配列）の
> **履歴方式**へ変更（蓄積・クリアしない = 集約時のクリア処理も削除で単純化）②一括選別の部分成功トーストが
> 失敗理由を落とす → 最後のエラーメッセージを添える ③タグバッジのマークアップが 3 か所コピー →
> 共通部品 `ImprovementsTagBadges` へ抽出。
> R7 = 再レビュー（高）で指摘 2 件（低）→ 全件反映: ①mock の unclusterRequest / setRequestAdoptionBulk が
> commit() の永続化可否を捨て、容量超過時も成功トーストのみ（submit / editRequest の handled パターンと乖離）→
> persisted を返し UI が警告 ②受付箱の行選択チェックボックスの aria-label が全行同一 →
> 要望本文の先頭を補間して一意化。
> R8 = 再レビュー（高）で指摘 3 件（低）→ 全件反映: ①受付箱の page / comments 列（スロット描画の仮想列）にも
> 飾りのソートヘッダ → sortable:false 追加 ②統合テストに R1 世代の旧コメント（「集約後は除外がクリアされる」）が
> 残留 → 履歴方式（蓄積）の記述へ修正 ③プロンプトのタグ読み方注記が文言ハードコード →
> IMPROVEMENT_REQUEST_TAG_META（SoT）から組み立てる形へ。
> R9 = 再レビュー（高）で指摘 3 件（低〜中）→ 全件反映: ①TagBadges が未知タグ値（DB は CHECK なし =
> アプリ層担保）で TAG_META 参照が undefined になり描画クラッシュし得る → 表示前に normalizeImprovementTags
> （buildCodingPrompt と同じ姿勢）②functional-requirements F-42-14 と shared 型ドキュメントに
> 「外すには取消のみ」の旧記述が残留 → 「集約の解除」を追記 ③一括選別の取り消しが 1 件ずつで非対称 →
> 「まとめて未選別に戻す」を追加（原則9.5）。
> R10 = 再レビュー（高）で指摘 4 件（低）→ 全件反映: ①一括選別で容量警告と部分失敗が同時発生すると
> 失敗件数の報告が消える → 併記 ②選別ガードの判定順が集約済み優先で、取消済み + 集約済み行に
> 「集約の解除」を案内 → 解除も取消済みで行き止まり（AKO-REQ-018）→ **取消済みを先に判定**（正しい
> 最初の一手 = 復元 を案内。テスト・コメントも更新）③選別列のソートが表示値（集約済みバッジ・
> 未定義補完）と食い違う並びになる → sortable:false ④R1 ノートの列名が旧名 unclustered_from のまま →
> 改称の経緯を明記。
> R11 = 再レビュー（高）で指摘 1 件（**中・実バグ**）→ 反映: mock の集約（mockGenerate）が 1 回の実行で
> 複数の新規 item を作ると `nextId`（コミット済み行のみ走査）が同じ id を重複採番
> （id キーの一括更新が別 item を巻き込む・v-for キー衝突。既存行だが本改修の一括採用・集約解除で
> 顕在化しやすくなった）→ 基点採番 1 回 + ローカル加算へ修正。
> R12 = 再レビュー（高）で指摘 2 件（低）→ 全件反映: ①/generate の追記先候補 SELECT が無順序で、
> 同一ページの triage item が複数あるとき（解除 → 再集約後の通常ケース）追記先が実行ごとに揺れ
> mock の作成順とずれる → ORDER BY created_at, id で決定化（原則6）②集約解除の取り消し
> （元 item への再アタッチ）フローが無い点が残課題に未記録 → 残課題へ登録（原則9.5）。
> R13 = 再レビュー（高）で指摘 3 件（低・防御/整合）→ 全件反映: ①generate の claim UPDATE が除外履歴を
> SQL で検証せず、LLM プラン作成（数秒）と claim の間の競合で履歴 item へ戻り得る →
> `AND NOT excluded_item_ids @> to_jsonb($1)` を両 claim へ追加（SQL 側最終防衛）②uncluster（request →
> item）と generate 追記（item → request）のロック順が逆でデッド50X の窓 → uncluster を item 先行ロック +
> 再検証（競合は AKO-REQ-020 の 409）へ ③受付箱のタグラッパが生 tags 長で表示判定し、未知値だけの行に
> 空の余白 → normalize 後の長さで判定。
> R14 = 再レビュー（高）で指摘 2 件（低・クリーンアップ）→ 全件反映: ①creates 側 claim の
> `NOT @>` 述語が新規採番 id に対して常に真（デッドコード + 誤解を招くコメント）→ 述語を appends 側
> のみに整理・コメント修正 ②一括選別の adoption 不正の早期 return が重複除去前の件数で failed を
> 報告（他経路と不一致）→ 重複除去後の件数へ統一。
> R15 = 再レビュー（高）で指摘 2 件（低・非ブロッカー）→ 全件反映: ①一括選別の部分失敗で選択が全解除され
> 失敗行を探し直す必要があった → failedIds を返し**失敗分だけ選択に残す**（トーストにも明記）
> ②API モードの一括選別が認可断（AKO-PRM-001）後も残り全件へ逐次 POST → 認可断で打ち切り。
> R16 = 再レビュー（高）で指摘 3 件（低〜中）→ 全件反映: ①部分失敗の failedIds 再選択が watch 完走後に
> 代入され、選別不能行の見えない選択が残留し得る → 代入時点の選別可能行で濾す ②行内 採用/不採用
> （setRequestAdoption 経由）だけ persisted を落とし容量超過で成功トーストのみ → persisted 伝搬 + 警告
> ③AKO-REQ-020 を uncluster も返すのに api-design が選別変更のみ記載 → コード表・エンドポイント行を更新。
> R17 = 再レビュー（高）で指摘 4 件（低〜中）→ 全件反映: ①部分失敗トーストの「選択に残しました」が
> 濾過後は空になり得て不正確 → 実際に残した件数で案内 ②mockGenerate だけ persisted を落とし
> 容量超過で集約結果が黙って消える → persisted 伝搬 + 警告 ③一括処理中もチェックボックスが操作でき、
> 完了時の選択上書きが途中の選択を消す → bulkBusy で無効化 ④認可断（403）後の refresh が
> catch の [] でキャッシュを空にし画面が白紙化 → 認可断時は最新化せず最後の表示を保持。
> R18 = 再レビュー（高）で指摘 3 件（低〜中）→ 全件反映: ①白紙化ガードが AKO-PRM-001 限定で 401/断線を
> 取りこぼす → refresh 自体を「失敗時は最後のデータを保持」へ（catch の [] を廃止）+ 認可/認証断は
> 逐次 POST・refresh とも打ち切り ②決着済み（解決済み/対応しない）item からも集約解除でき、判定済み
> 記録のトレース書き換え + 実装済み内容の再集約が起こり得る → AKO-REQ-021 で禁止（共有ガード拡張・
> UI もボタン非表示 + reopen 案内・統合テスト固定。先に reopen すれば解除できる = 原則9.5）
> ③buildCodingPrompt がタグ normalize を要望ごとに 2 回実行 → 1 回に集約。
> R19 = 再レビュー（高）で指摘 1 件（低）→ 反映: 一括選別の打ち切り判定にネットワーク断
> （AKO-GEN-NET）が含まれず、接続断で選択件数分のタイムアウトを直列に待つ → 全滅確定エラーの判定を
> isFatalBulkError（認可断・認証断・ネットワーク断）へ一元化し逐次 POST・refresh とも打ち切り。
> R20 = 再レビュー（高）で指摘 3 件（低・クリーンアップのみ「バグではない・非ブロッカー」判定）→ 全件反映:
> ①一括選別 mock 経路の Set(applicable) 二重構築 → 1 回に ②重複除去式の二重記述 → 検証前に 1 回へ
> ③受付箱のタグ表示で normalize が呼び出し側と部品側の 2 回 → 空判定・外枠（wrapperClass）を
> TagBadges へ集約し呼び出し側の normalize を撤去。
> R21 = 再レビュー（高）で指摘 2 件（低〜中）→ 全件反映: ①行内 採用/不採用 に busy ガードが無く、
> API モードの連打で「もう一度押すと未選別に戻す」トグルが空振り（同じ選別を二重送信）+ 一括実行中も
> 行ボタンが押せた → adoptionBusy ガード + 行・ドロワーのボタン無効化 ②集約解除の確認・案内文が
> 「元の改修単位へは戻らない」不可逆性を伝えず原則9.5 の趣旨に不足 → 確認ダイアログ・ドロワー文言に明記。
> R22 = 再レビュー（高）で指摘 1 件（低・効率）→ 反映: 1 件選別（API モード）がクリックごとに
> 4 コレクション全再取得（refresh）で、行内選別のホットパスが重い → 応答行（RETURNING）を
> キャッシュへ差し込む方式へ（画像は遅延ロード済みを引き継ぐ。一括選別の「反映 1 回」と同じ姿勢）。
> R23 = 再レビュー（高）で指摘 1 件（低・再利用慣行）→ 判断つき対応: 新設の busy フラグ 3 つが
> useAsyncAction（2026-07-21 #3）を使っていない指摘。本ページは既存の busy 管理 6 か所
> （commentBusy / noteBusy / rejectBusy / requestEditBusy / generating / promptBusy）がすべて手動 ref +
> 素の btn クラスで統一されており、新設 3 つだけ useAsyncAction + UiButton :loading へ移すと 1 ページ内に
> 2 様式が混在するため**ページ内慣行への統一を維持**（全面移行は本ページの既存 busy 群を改修する
> タイミングで一括実施 = 原則9.5 の遡及適用ルールと同じ扱い）。指摘の実害（実行中の無言化）には
> 一括選別バーへ「処理中…」（role=status）の可視フィードバックを追加して対応。
> R24 = 再レビュー（高）で指摘 2 件（低）→ 全件反映: ①API モードの一括選別が apiWrite 直呼びで、
> 途中打ち切り時に成功分のキャッシュ反映が消える（成功トーストと表示が矛盾）→ 1 件 API
> （setRequestAdoption = 応答行を即キャッシュ反映）の再利用へ・refresh は失敗があった場合のみ 1 回
> ②取消済み item からの解除がガードされず論理削除中のトレースを書き換え得る → AKO-REQ-022
> （先に item を復元。共有ガード・UI ゲート・単体/統合テスト・ドキュメント登載）。
> R25 = 再レビュー（高）で指摘 2 件（低）→ 全件反映: ①ドロワーの選別ボタンが bulkBusy を見ず、
> 一括実行中に同一要望へ単発選別が競合し得る（R21 の行ボタン対応の取り残し）→ 両フラグで無効化
> ②プロンプトのタグ凡例が item ごとに重複出力 → プロンプト冒頭に 1 回へ（normalize も全体 1 パスに）。
> R26 = 再レビュー（高）で指摘 1 件（低・冗長のみ「正しさへの影響なし」判定）→ 反映:
> unclusterRequest 内で同一テーブル（improvementItems）の tbl() 参照が 2 つ → 1 つに統合。
> R27 = 再レビュー（高）で指摘 2 件（低・いずれも正しさへの影響なしと明記）→ 判断つき受理で決着:
> ①一括選別の逐次直列 POST（大規模選択で遅い）= fatal 打ち切り・キャッシュ整合を単純に保つ**文書化済みの
...
**残課題（本改修で新設・未対応）:**
- タグ（壁打ち/お任せ）の投稿後の単体編集（F-42-16 の本文編集に相当するタグ版）。現状の取消フローは
  送信直後ビューの「取り消す」→ 再送（投稿直後で付随記録が無いため実害なし）。管理側で意思表示の
  変更が必要になったタイミングで editRequest の拡張（tags 部分更新）として対応する。
- 集約解除の取り消し（元 item への再アタッチ = 原則9.5）。解除履歴（excluded_item_ids）は蓄積式のため、
...
- [x] R2 = 再レビュー（両ロール）。**R1 対応③の修正不備 MAJOR 1（両ロールで一致）+ MINOR 3 + NIT 1 → 全件対応**:
  ①（MAJOR）フェイルクローズが 4 ページ中 2 ページで実際には閉じていなかった（workflow = `tab !== 'routes'`
  のフォールバック分岐で「自分の申請」一覧が描画・improvements = `v-else` で「改修案件」一覧が描画）→
  タブ内容の分岐を全ページ「タブキーの明示指定のみ」へ（workflow = mine/pending/all の列挙 + routes 明示・
  improvements = `v-else-if="tab === 'items'"`）。E2E に「全タブ deny → 空状態のみ・一覧/経路設定 非描画・
  解除で復帰」の 3 チェックを追加（28 チェックへ）
  ②（MINOR）リンク化規則の厳格化が F-42-20 / screen-design / api-design の旧記述（「先頭 `/` のみ」）のまま →
  3 箇所へ isInternalPagePath / normalizeImprovementPagePath の仕様を明記
  ③（MINOR）フェイルクローズ挙動の要件・画面設計への追記漏れ → F-16-8・screen-design 2 箇所へ明記
  ④（MINOR）未集約の添付フェッチで応答に載らない stale 行（並行集約等）が逐次リフェッチのループになる →
  要求元行の id も成功時にマークして収束させる（応答 = 未集約全件の正のため取りこぼしなし）
  ⑤（NIT）AIの参照範囲タブ脚注の「既定値」がレイヤ内参照と読める → 「どのレイヤでも決まらない場合」へ修正
- [x] R3 = R2 反映後の再確認（独立ロール）。R2 対応 5 件すべての正当性・4 ページのフェイルクローズの実効性・
  E2E 28 チェックの実 PASS・退行なし（両単体 + typecheck green）を確認。検出は screen-design の
  添付遅延ロード表現の同期漏れ NIT 1 件のみ → 修正案どおり反映（F-42-20 と同一表現へ）し、
  **未解決の指摘ゼロで完了**
...
- [x] R1 = 独立ロール 2 体（コードレビュアー + システム監査官）の並行レビュー。
  **コードレビュアー: MAJOR 1** = 一般利用者が自分の取消済み要望を復元できない（取消でドロワーが閉じ、`activeRequests` は
  `!archivedAt` で除外・API も `includeArchived=canManage` で取消済みを返さないため、取消した本人に復元導線が無く原則9.5 と
  確認ダイアログの「いつでも戻せます」に反する）+ **MINOR 1** = ImprovementSubmit が上限超過本文を無言で切り詰めて送信
  （編集フォームは `over` でブロックするのに投稿は未ブロック）。**システム監査官: 重大なし**（マイグレーション冪等・非破壊、
  サーバー権限ガード維持、週報/月報パリティ、AI 参照範囲の非供給を実コードで確認）+ LOW 3。
  **全件修正:** ①一般利用者の GET /requests を「有効な全要望 + 自分の取消済み」に拡張し、取消ドロワーは閉じずフッターを
  「取消を戻す」へ切替 + 一般受付箱に「自分の取消済みを表示」トグル + 復元ボタンを追加（原則9.5 の復元導線を回復）
  ②ImprovementSubmit の送信を `bodyOver` でブロック（disabled + 早期 return。編集フォームと同一判定）
  ③LOW: GET /requests のトリアージ状態（adoption・excludedItemIds）を非管理者へは返さない + 詳細ドロワーの選別バッジも
...
## 91. 改修依頼 2026-08-20（9 改修単位 = 顧客関係グラフ/営業・BP活動再編/週報マトリクス/BP項目/顧客コンテキスト/日報リマインド/通知連携/その他トグル/改善要望4改修）の完了条件（Definition of Done）

対象: mockup（Nuxt 4 SPA）+ api（Hono/PostgreSQL）+ shared/domain の全レイヤー。マイグレーション 0073〜0076。

### 91-1 顧客関係(会社)のノードグラフ化（#1）
...
### 91-2 営業活動・BP活動の「案件ヘッダー + 活動ログ」再編 + 登録バグ修正（#2/#4）
...
- [x] R2 = R1 修正の独立再検証。R1 修正 13 項目は**全て正しく適用**と確認。新規指摘 = MAJOR 1（同クラスの
  ...
  状態遷移の同期 LLM）+ MINOR 1（§91-2 の並び順記載）+ NIT 4（ロック解放ガード・接続取得位置・
