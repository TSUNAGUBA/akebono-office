...
- **UI 改修（本 PR）:** 役職マスタ管理・部署の任意化（未所属登録/配属解除）・モック実装ページへのバッジ表示・マスタ部分 PATCH 修正（PR #19）後続
...
## 3. 今回バッチ（UI 改修）の完了条件（Definition of Done）

- [x] 役職マスタ管理ページ（`/masters/titles`。汎用区分 category=title の専用ビュー）+ 初期値マイグレーション 0004
- [x] メンバー登録の部署を任意化（未所属で登録可。select の空値ラベル対応 = emptyLabel）
- [x] 部署画面から配属解除（未所属へ戻す）
- [x] API モードでモック実装ページ・機能に「モックアップ」バッジ（ページヘッダー自動判定・カードメニュー・メニュー一覧・inbox エスカレーションタブ・日報 AI アシスト）
- [x] UiSchemaForm の select に aria-label 付与（アクセシビリティ = U-5）
- [x] 検証: API 統合テスト 34 / API モード実クリック E2E 50 チェック（4 スイート）/ モック回帰（typecheck・35 tests・generate・実クリック）
- [ ] バッチ3 以降（エスカレーション・ワークフロー・シフト・AI アシスタント・カレンダー連携）: 優先順はオペレーターと協議
