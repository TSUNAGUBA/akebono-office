...
- [x] **排他制御（C-1）**: 棚卸の差分計上は残高の read-then-post のため、`akebono-trade` から `balanceOf`・`lockInventoryKeys` を
  **export して再利用**（原則3）。第1パス（解決・検証・DB read）→ 対象 SKU×倉庫キーを**ソート取得の advisory lock**（`lockInventoryKeys`）
  → 第2パス（差分計上・追記）の順で、手動棚卸・出荷/移動および他取込元の在庫取込と直列化（キー順一致でデッドロック回避。
  ラン全体は `imprun:sourceId` ロック内 = 同一取込元は直列）。
...
- [x] api 統合 **242 passed**：ⓐ 手動棚卸に**マイナス実棚数**（wh-01 実棚 −3〔理論 6〕→ −9 調整・残高 −3。後続用に 6 へ復元）・
  ⓑ **棚卸 CSV 実取込**（実物理在庫数 50 → 残高 50・同一ファイル再取込は冪等スキップ・−5 取込で残高 −5）を追加。
