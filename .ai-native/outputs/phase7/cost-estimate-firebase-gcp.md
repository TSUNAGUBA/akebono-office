...
|---|---:|---:|---:|
| **A. 節約構成**（共有コア DB・AI 控えめ） | 約 $60 | **約 9,700 円** | 約 195 円 |
...
| Cloud Run インスタンス時間 | ポーリングにより営業時間中は常時 1 台ウォーム（60 秒間隔ではゼロスケールしない）: 在席 9h + スケールダウン猶予等 1h = 10h×21日 = 210h + ピーク/ジョブ/AI 実行 40h | **約 250 h（90 万 vCPU 秒）** |
| LLM 呼び出し | チャット 2,100 + ドラフト 1,050 + AI コメント 2,100 + インサイト 224 + AI カンパニー 480（= 40タスク×〔分解1+割当1+遂行5〕+ Web 調査 40×5） | **約 6,000 回** |
...
- **SLA の注意: Cloud SQL の SLA は HA 構成（案3）のみが対象**。ゾーン構成は共有コア（案1）・専有コア（案2）とも SLA 対象外（[公式 SLA](https://cloud.google.com/sql/sla) が単一ゾーン・共有コアを明示的に除外）。案2 を推奨する根拠は SLA ではなく、専有 vCPU / メモリ 3.75GB による性能・安定性（勤怠打刻という記録系 SoT を支えるワークロードへの余裕）。HA（案3）は「日中数十分の停止も許容できない」「SLA が契約上必要」な場合のみ。ゾーン構成でも自動バックアップ + PITR で RPO は確保できる。
- DB 容量: 現行設計はアーカイブジョブがなく記録系 + bytea 添付（ナレッジ・ノート・AI タスク添付は常に DB 内）が単調増加する。年 +10〜20GB → **+約 $3.3〜6.6/月/年**（SSD $0.221 + バックアップ $0.104/GiB）の逓増を見込む。
...
- 月 250 インスタンス時間（1 vCPU / 512MiB）= 90 万 vCPU 秒・45 万 GiB 秒
  ...
  - メモリ: 450,000 GiB 秒 ≒ 無料枠 450,000 → **$0**（無料枠とちょうど同値。稼働が伸びて超過しても $0.3/月 規模）
...
- 幅: 思考トークンが少なければ $12、AI 利用が想定の半分（思考込み実効 2 倍のまま）なら約 $10（シナリオ A）、AI カンパニー 100 タスク + 思考上振れで $45 程度（シナリオ C）。
...
| Secret Manager | シークレット 3 個 < 無料枠 6 バージョン。アクセスはインスタンス起動時のみ（ローテーション時に旧バージョンが蓄積すると $0.06/版だが軽微。destroy 運用を推奨） | $0 |
...
| Cloud Run | $12 | $12 | $18 |
| Vertex AI | $10 | $20 | $45 |
| Cloud Storage + egress（+ B/C は予備費含む） | $1.2 | $2 | $4 |
| **合計 (USD)** | **約 $60** | **約 $105** | **約 $206** |
| **合計 (JPY @162.5)** | **約 9,700 円** | **約 17,000 円** | **約 33,500 円** |

> B / C 最終行の積算値は $1.2（GCS $0.6 + egress $0.6）。差分は Artifact Registry / Hosting 超過・メモリ課金・ストレージ逓増初年度分等の少額変動を見込んだ予備費。
...
- 参考: RDS が安いのは共有コア（t4g.micro = 2vCPU バースト・1GB RAM）のため。Cloud SQL 側も共有コア（案1）を選べば価格は近づくが、いずれも SLA 対象外である点は同じ（なお Cloud SQL はゾーン構成なら専有コアの案2 も SLA 対象外。SLA が付くのは HA 構成のみ = §4.1）。
...
| 要因 | 影響 | 備考 |
|---|---|---|
| **AI カンパニーの利用量** | 1 タスク（5 ステップ・グラウンディング込み）≈ $0.07〜0.11。100 タスク/月で +$7〜11 | 将来拡張の「AI 社員の実実行」（LLM 自律実行）が入ると呼び出し数が跳ねるため**再試算必須** |
...
1. **移行する場合は案2（db-custom-1-3840 ゾーン + プライベート IP）を推奨。** 月額 約 17,000 円・現行比 +約 7,100 円で、クロスクラウド運用の複雑さ（NAT・SG・CA・二重 IAM）を撤廃できる。勤怠打刻という記録系 SoT を扱う以上、共有コアは性能・安定性の面で本番非推奨（なお SLA はゾーン構成の案1・案2 とも対象外で、案2 推奨の根拠は専有リソースによる安定性）。
2. **HA は初期導入不要。** 50名の社内アプリはゾーン構成 + 自動バックアップ + PITR で十分。稼働要件が上がった時点で HA へ引き上げ（設定変更のみで可能）。**SLA が契約上必要な場合は HA（案3）が唯一の選択肢**（Cloud SQL の SLA はゾーン構成に適用されない）。
