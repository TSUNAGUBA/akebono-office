...
設計（確定）:
- 判定 SoT = `mockup/app/utils/team-visibility.ts`（純関数）。未設定 = 既定表示（マトリクス = 社員・契約・アルバイト / タイムライン = 全員 = 従来どおり）・設定あり = 「選択メンバー + 自分」でマトリクス・タイムラインを統一（7h の候補外特例は廃止）。F-16-6 参照権限は常に別途適用
...
- 下位互換注意: 設定を保存済みの環境では、設定に含まれない取締役・外注がタイムラインに出なくなる（設定モーダルで追加 = 回復フロー。configs 形式は不変 = パッチ不要）
