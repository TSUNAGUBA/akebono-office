...
- **マイルストーン達成: 全ドメインの API 接続が完了し、mock-status は空 = API モードのモックバッジ全廃**。
  最後まで未移行だったドキュメント管理も**バッチ7l（§18）で本実装済み = 全ドメイン移行完了**
- 旧セッションの scratchpad（E2E スタック 16 スイート）は**コンテナ再作成で消失**。
  バッチ6b で E2E ハーネスを再構築し、現在は**リポジトリ直下 `e2e/` にコミット済み**（検証スタック節参照）。
  現在のスイート: batch6b（12）+ batch6c（16）+ batch6d（11）+ chatbot-multiturn（10）+ team-visibility（10）+ モック回帰（10）
...
## 17. バッチ7k 設計メモ（オペレーター指示 2026-07-19 #13）— **実装済み（implementation-status §31 が SoT）**
...
## 18. バッチ7l 設計メモ（オペレーター指示 2026-07-19 #14）— **実装済み（implementation-status §32 が SoT）**

要求: ファイル管理の本実装。①保管 = Firebase の Cloud Storage ②Google ドライブ連携 ③保管ドキュメントを各所の AI 参照対象に ④最適化インデックス + ベクター化 ⑤必要に応じてファイル URL を案内してダウンロード。

設計（確定）:
- メタ = `documents`（0027）/ 実体 = GCS（STORAGE_BUCKET・raw fetch + ADC = 依存追加なし）or `document_blobs` bytea フォールバック（未設定環境）。署名 URL = IAM signBlob の V4 実装（lib/storage.ts。純関数 buildV4SignParts が単体テスト対象）
- ドライブ = calendar.ts の OAuth 共用（SCOPES に drive.readonly 追加 = **既存連携者は再接続が必要**。旧スコープは AKO-DOC-006）
- AI 参照 = search_docs `source_kind='document'`（buildSearchDocs にループ追加・チャットボットは can('documents') ガード + 署名 URL 添付・AI カンパニーは prepareStepExecution の材料に上位 3 件）
- テスト注意: 統合はフォールバック（STORAGE_BUCKET=''）で全経路を検証。インデックス検証は POST /v1/search/reindex（デバウンス待ち回避）。GCS/Drive の実通信はテスト対象外（署名の正規リクエストのみ単体で検証）
