...
| F-01-2 | 通知フィード | クリック・既読化・カテゴリタブ | 未読通知・エスカレーション・承認待ちの横断表示（直近 5 件）。**タブでカテゴリ分け（オペレーター指示 2026-07-22）: すべて / エスカレーション（kind=escalation）/ 承認依頼（kind=approval のうち稟議以外）/ 稟議（リンク先が `/workflow` の通知）** | F-12 へ遷移 |
...
| F-13-9 | ダッシュボードレイアウト（表示・配置カスタマイズ・オペレーター指示 2026-08-03） | ダッシュボード（F-01-4）の表示・配置をテンプレート（5 種・プレビュー付き）から選択、または**セクション構成を手動編集**（追加・削除・改名・並び替え・カード割当）。**設定・セクション配置とも 3 階層で解決**: ユーザー設定（`user_preferences.dashboardLayout`・本人/端末間同期）> テナント設定（`app_configs.dashboard-layout`・管理者が全社既定を設定）> デフォルト表示。テナント未設定時は従来の `menu-categories-dashboard`（F-13-8）を default options と組み合わせて下位互換解釈（原則7）。保存値 = `DashboardLayout`（templateId + sections + options{notifications / showAkebono / density}）を JSON（手動編集時は templateId=`custom`・options は現行維持）。**UI = ダッシュボードヘッダのモーダル**（「テンプレート」/「セクションを編集」タブ・スコープ選択付き。自分向けは全ユーザー可・全社既定は管理者のみ）。セクション編集の割当候補 = 基本メニュー + 外部リンク + AKEBONO 業態アプリ。**取消フロー（原則 9.5）= 層ごとに「設定を解除」可**。既存 API（`/v1/me/preferences` の value 4KB 上限内・`/v1/configs`）のみ利用し新規 API は不要 |
