...
| F-06-7 | AI アシスト入力（オプション） | カレンダー予定・ぽいぽいメモ（低摩擦の断片投稿）・AI ヒアリング回答（予定 1 件につき 1 問 + まとめ 3 問） | 蓄積ログ + 予定から AI が日報ドラフトを生成 →**本人が確認・修正してから提出**（提出済みは上書きしない）。入力方式は設定で 通常フォーム/AI アシスト/両方 を選択（F-13-7） | 日報ドラフト（entries=予定から工数・PJ 推定 / 所感・課題・明日=回答から抽出。生成根拠を明示） |
...
| ID | マスタ | 主要項目 |
|---|---|---|
| F-10-1 | メンバー管理 | 氏名・雇用区分（取締役/社員/契約社員/アルバイト/外注）・**勤務体系（勤怠ルールの個別指定。未指定なら雇用区分の既定を適用）**・部門・役職・入社日・週所定日数/時間（有給比例付与に連動）・ロール（admin/member）・打刻対象フラグ |
