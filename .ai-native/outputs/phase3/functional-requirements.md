...
| F-06-3 | 提出状況一覧（チームタブ） | 期間 | メンバー×日のマトリクス。**バッチ7h（オペレーター指示 2026-07-19 #10 ①）で全員へ公開**: 表示メンバーは「表示メンバー設定（configs `teamVisibleMemberIds`・管理者が歯車から設定・自分は常に表示。**バッチ7k で候補 = 在籍中の全メンバー（取締役・外注含む・雇用区分バッジ付き）・空 = 既定表示（マトリクス = 社員・契約・アルバイト / タイムライン = 全員）**）∩ 日報参照権限（F-16-6）」。一般メンバーには他人の下書きの存在・内容を見せない（提出済み/未のみ。API scope=team も提出済みのみ返す）。未提出リマインド送信・下書き表示・工数乖離は管理者（乖離は HR 含む）のみ | 提出状況・リマインド通知（管理者） |
