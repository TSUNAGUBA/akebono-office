...
| ID | 機能 | 入力 | 処理 | 出力 |
|---|---|---|---|---|
| F-18-1 | 顧客ログの記録 | 日付（必須）・**開始/終了時間（任意。分は 15 分単位から選択 = UI 制約・終了は開始必須 + 開始より後 = 2026-07-31）**・**属性タグ（商談/取材/イベント等のプリセット + 自由入力・最大 10 件 = 2026-07-31）**・顧客(会社)（必須・**コンボボックス = 2026-07-31**）・担当者(人)（任意・**コンボボックス**・会社所属を検証）・**自社の担当者（必須・既定 = ログインユーザー = 2026-07-31）**・件名（任意）・**担当者メモ / 議事録メモ（どちらか必須 = 2026-07-31。旧「会話内容」は担当者メモへ引き継ぎ）** | 本人所有の記録系（追記 + 本人編集〔監査ログ〕+ 取消/復元 = 原則2/9.5）。**コンボボックスの未登録名は保存時に companies/contacts へ新規マスタ登録して反映（同一トランザクション・正規化名一致は既存へ名寄せ・監査ログ。一般メンバーでもこの経路では登録可）** | 顧客ログ一覧（日時・会社・担当者・タグ・冒頭）→ 詳細モーダル |
