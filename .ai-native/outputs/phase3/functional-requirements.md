...
## F-40 メディア分析（`/media`。AKEBONO 業務配下のアプリ = 2026-07-28 追加。本実装 = 同日）

Akebono 業務の事業セグメント（業態）と 1:1 で対になるオウンドメディアの分析・記事生成。
モック = 擬似 OAuth + 記事インベントリからの決定的導出 / API = Google OAuth 2.0（analytics.readonly・
セグメント単位）+ GA4 Data/Admin API の実データ（詳細は api-design §3 の /v1/media 行と phase7 実装状況 §37）。

| ID | 機能 | 入力 | 処理 | 出力 |
|---|---|---|---|---|
| F-40-1 | メディア分析ハブ | 業態選択 | 現在業態の GA 連携状況・主要指標サマリー + 全業態の一覧 | `/media`（機能カード + 業態別サマリー） |
| F-40-2 | メディア分析（GA × AI） | 業態・期間（既定 28 日） | GA 由来指標（セッション・PV・CV・チャネル/デバイス/日別/記事別/セクション別）の可視化 + AI インサイト（サイト構成・記事・次アクション。生成 → 保管 → 再生成で上書き）。業務 × メディア PDCA タブ = 売上と流入の突合（相関・PDCA・アクション） | `/media/analytics`（KPI・チャート・テーブル・AI インサイト） |
| F-40-3 | AI 記事生成スタジオ | お題・キーワード・目的・質・トーン・読者 | 記事ドラフト生成（API = Vertex AI → 決定的フォールバック）。過去分析からのお題提案。採用でコンテンツ資産（記事インベントリ）へ登録 = 分析対象に反映。取消・復元・採用取消可（原則9.5） | `/media/articles`（生成フォーム・プレビュー・履歴） |
| F-40-4 | メディア設定（管理者） | 業態・GA 連携操作・AI 分析設定 | GA 連携（モック = 擬似 OAuth / API = OAuth 同意 → GA4 プロパティ選択）と AI 分析設定（サイト名・目的・読者・トーン・キーワード）の保存（部分更新）。連携解除可 | `/media/settings` |
