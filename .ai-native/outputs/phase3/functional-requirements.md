...
| ID | 機能 | 入力 | 処理 | 出力 |
|---|---|---|---|---|
| F-16-1 | 権限ルール管理 | レイヤ（ロール/役職/個人）・対象・リソース（機能 or マスタ項目）・項目キー・効果（許可/拒否） | 権限ルールの CRUD（管理者のみ・監査ログ記録・論理削除） | `/masters/permissions` のルール一覧・編集 UI |
