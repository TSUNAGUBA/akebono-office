...
**コックピット化（F-01 改訂「夜明けの管制塔」。2026-07-29 オペレーター承認。設計 SoT = `phase5/cockpit-design.md`）:**
ダッシュボードは「今日の状態（フレーム）→ 次の一手（エンジン）→ 計器 → 着地予報（地平線）」の 3 層コックピット + 従来のカードメニュー（格下げ配置）・通知フィードで構成する。売上サマリは 売上管理（F-15）、稼働状況サマリは 提供システム稼働状況（F-11）の独立ページのまま、打刻はヘッダーの「タイムカード」モーダル + 次の一手への PunchClock 埋込。

| ID | 機能 | 入力 | 処理 | 出力 |
|---|---|---|---|---|
| F-01-4 | コックピット・ストリップ | 表示（punchState × JST 時刻）・予報行タップ | フェーズ導出（punch 対象外 = 表示なし / 出勤前 / 勤務中 / 休憩中 / 退勤済み）+ 今日の完了メーター（一手 0 件の日は非表示） | 挨拶・日付・フェーズバッジ・完了メーター・着地予報 1 行（F-01-7 の開閉） |
| F-01-5 | 次の一手 | 表示・実行ボタン/PunchClock | 優先順位キュー（未退勤打刻→出勤/退勤→稟議承認→勤怠承認→日報→シフト希望→エスカレーション→有給5日。権限・機能トグル・ロールでゲート） | 最優先 1 件の大カード + 最大 4 行 + 「ほか N 件」。全消化は ok トーンの完了表示 |
| F-01-6 | 計器（ロール別 KPI） | 表示 | 自分（実働/有給残/36協定）・シフト（parttime）・労務（hr/admin。件数のみ）・経営（sales 権限）・事業（Member.segmentIds）+ 週次インサイト 1 行（保存済みのみ）。該当なしは枠ごと非表示 | UiKpiCard 群（各機能ページへ遷移） |
| F-01-7 | 着地予報 | ストリップの予報行タップで開閉（既定閉） | goals マスタ × 実績から landing-forecast（純関数）で着地予測（量 = 日割り外挿 / 率 = 外挿しない / 残業 = 上限型）。トーンは ok/warn のみ | 滑走路バー 1〜4 本 + reason 1 行 + 針路修正 1 件。goals 未設定は admin へ `/masters/goals` 導線 |
| F-01-1 | カード型メニュー | クリック・カテゴリチップ | カテゴリ（既定: 意思決定支援 / AKEBONO / 業務ツール / AIネイティブカンパニー / 経営・状況 / 業務支援 / 管理）。全遷移先を網羅。**カテゴリはカスタマイズ可（F-13-8 = バッチ7h）**: チップで選択したカテゴリのみ表示（「すべて」あり・選択は sessionStorage 記憶）。未割当メニューは自動的に「その他」へ。**コックピット化に伴い最下部へ格下げ配置（挙動は不変）** | 各機能へ遷移。バッジ（承認待ち件数等）表示 |
...
> **改訂メモ:** 旧 F-01-1（売上サマリ）は F-15 売上管理へ、旧 F-01-2（稼働状況サマリ）は F-11 へ、旧 F-01-3（打刻）は F-01-3（ヘッダーモーダル）へ移行。F-01-4〜7 は 2026-07-29 のコックピット改訂で追加（詳細仕様は `phase5/cockpit-design.md` が SoT）。
>
...
| ID | 機能 | 入力 | 処理 | 出力 |
|---|---|---|---|---|
| F-06-1 | 日報作成 | **テーマ**（自由入力。旧称: 業務テーマ → 2026-07-22 オペレーター指示で改称）別エントリ（**内容**（旧称: 作業内容）・**時間**（旧称: 工数）0.25h 刻み・進捗。**通常入力は共通ヘッダ 1 行のテーブル形式で各セルへ入力（2026-07-22）**）+ 所感・課題・**明日の予定（テーマ/目的/内容/時間・最大 3 件。翌営業日の日報エントリへ自動反映。旧自由記述 tomorrow は互換表示）**。日付ナビは上段 ←/今日/→・下段 日付直接選択。**表示モード「週」「月」を切替可（月は 1 日〜末日の横スクロール / カレンダー形式の 2 ビュー = 2026-07-22）** | 下書き→提出。時間合計と勤怠実労働の乖離チェック。旧データ（projectId）は表示・編集時にプロジェクト名のテーマへフォールバック | 日報。乖離時は警告 |
| F-06-2 | 週報作成 | 今週の目標達成・主要業務・課題・来週予定 | 日報からのプリフィル | 週報 |
| F-06-11 | 全員の週報（オペレーター指示 2026-07-22 追加） | 対象週・部署・メンバーで絞り込み | 全メンバーの**提出済み**週報の週次一覧（下書きは本人以外に見せない）。参照可否は日報と同じ権限表「日報・週報の参照対象」（F-16-6）で管理。API は `GET /v1/reports/weekly?scope=all` | 名前・主要業務サマリーの一覧 → 押下で詳細ドロワー |
...
| F-06-9 | 全員の日報（2026-07-17 オペレーター指示追加） | 対象月・**部署・メンバーで絞り込み（2026-07-22）** | 全メンバー・AI 社員の**提出済み**日報の月次一覧（下書きは本人以外に見せない）。参照可否は権限表「日報・週報の参照対象」（F-16-6）で管理 | 日付・名前・サマリー・時間の一覧（PC = 行 / モバイル = カード）→ 押下で詳細表示 |
