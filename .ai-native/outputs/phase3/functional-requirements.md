...
| F-16-7 | AI業務アシスタントの参照対象（オペレーター指示 2026-07-21） | レイヤ・対象・参照対象メンバー・効果（参照可 / 参照不可） | **誰の AI業務アシスタント（計画・振り返り）を readonly 参照できるかをレイヤごとに設定**（`PermissionRule` の擬似フィールド `member:<対象メンバー id>`・resource='ai-assistant'。allow = 参照可。**未設定 = 参照不可 = 許可制**（日報 F-16-6 と対称だが既定が逆 = 個人の作業計画のため）。**自分は常に参照可**）。適用範囲: `/ai-assistant` の対象メンバー切替（readonly 表示）・API の他メンバー計画/ログ取得（`GET /v1/task-plans?memberId=` `GET /v1/assist/logs?memberId=`・未許可 403 AKO-PRM-002）。マトリクスは対象外（ルール一覧のみ = F-16-6 と同じ設計判断） | ルール一覧の擬似リソース「AI業務アシスタントの参照対象」（対象メンバーを論理名で複数選択 = 1 名 1 ルール） |
