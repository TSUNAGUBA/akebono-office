...
| F-04-12 | 勤怠承認経路の設定（管理者。オペレーター指示 2026-07-30） | 区分（直行/直帰・打刻修正）× 承認ステップ（上から順・直列。承認者ロール = 管理者/人事/取締役/代表 または個人指定） | 稟議 F-07-5 と同型の経路設定（**金額帯は持たない**）。申請時に経路を凍結（route_snapshot）。区分に有効経路が無ければ**管理者 1 名の単段承認へフォールバック**（下位互換） | 区分ごとの経路一覧・追加/編集/無効化（復元可） |
