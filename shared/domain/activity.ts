 ...
 * 受付日 → 受付時刻 → 種別 → 件名 → 内容 → 優先度 → ステータス → 完了日 → 完了時刻（完了日必須）→ 会社
 */
 ...
 * 商談名 → 種別 → フェーズ → 金額 → 確度 → 受注予定日 → Next Action日 → 会社
 */
...
/**
 * ビジネスパートナー活動の入力検証（宣言順 = API/モック共通の適用順）。
 * パートナー → テーマ名 → 活動区分 → ステータス → Next Action日
 * （関連商談 relatedSalesActivityId の実在検証はデータ層の責務 = API は FK/存在チェック・モックは各 composable）
 */
export function partnerActivityError(input: PartnerActivityInput): string | null {
  return (capCodePoints(input.partnerName.trim(), ACTIVITY_NAME_CAP) ? null : 'パートナーを入力してください')
    ?? activityRequiredTextError(input.theme, 'テーマ名')
