/**
 * 日報 AI アシスト API（F-06-7）。home useReportAssist の API 版。
...
export function assistRoutes(pool: pg.Pool, env: Env): Hono {
  ...
  // 入力テキストの「AIで整形」（改修依頼 2026-08-20 → 文章補完へ強化 2026-08-21。認証済み全員可 =
  // 要望フォーム等の入力サポートの汎用型）。LLM（Vertex 構造化出力）を一次に使い、無効環境・失敗・空出力は
  // shared の決定的ヒューリスティック（改行・箇条書きの整形のみ）へフォールバックする（report-draft と
  // 同型 = 原則3/4。**表現の補完・言い回しの修正は LLM の責務** = フォールバックでは行わない・文書化済み）。
  // 結果は保存しない（何度でもやり直せる）。応答 { text, llm }: llm=true は LLM 整形・false はルールベース
  app.post('/format-text', async (c) => {
    ...
    const llmOut = await generateJson<{ formatted?: unknown }>(env, {
      ...
      system: 'あなたは社内業務アプリの改善要望を清書するアシスタントです。与えられたテキストを、'
