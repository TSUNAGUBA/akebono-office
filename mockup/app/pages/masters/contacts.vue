...
const crud = useMasterCrudAsync('contacts', 'p')
...
const { page, pageSize, rows: pagedContacts, total } = useListView<Contact>({ source: filtered })
watch([search, statusFilter], () => { page.value = 1 })

const tableRows = computed(() =>
  pagedContacts.value.map(c => ({
    ...c,
...
<template>
  <MastersMasterShell title="顧客(人)マスタ" description="所属会社・キーパーソン度（★1-3）・連絡先・メモを管理します">
    ...
    <UiSectionCard :title="`顧客(人)一覧（${total}件）`" flush>
      <UiDataTable
        :columns="columns"
        :rows="tableRows"
        clickable
