...
/**
 * F-10-2 業界マスタ（管理者専用）
 * シンプルな一覧 + 追加/編集（名称・表示順）。業界は直交軸で管理（複合値を作らない）。
 */
...
const { page, pageSize, rows: pagedIndustries, total } = useListView<Industry>({ source: filtered })
watch([search, statusFilter], () => { page.value = 1 })

const tableRows = computed(() => pagedIndustries.value as unknown as Record<string, unknown>[])

const columns: TableColumn[] = [
  { key: 'name', label: '業界名', primary: true },
  { key: 'displayOrder', label: '表示順', align: 'right', width: '90px', primary: true },
  { key: 'active', label: '状態', primary: true },
]

function asIndustry(row: Record<string, unknown>): Industry {
...
const selected = computed<Industry | null>(() =>
  selectedId.value ? ((crud.byId(selectedId.value) as Industry | undefined) ?? null) : null,
)
const form = ref<Record<string, unknown>>({})
const errors = ref<Record<string, string>>({})

const drawerTitle = computed(() =>
  mode.value === 'create' ? '業界を追加' : mode.value === 'edit' ? '業界を編集' : '業界詳細',
)

const formFields: FieldDef[] = [
  {
    key: 'name', label: '業界名', type: 'text', required: true, placeholder: '例）小売',
...
function openCreate(): void {
  ...
  const maxOrder = Math.max(0, ...(crud.list.value as Industry[]).map(i => i.displayOrder))
...
async function save(): Promise<void> {
  ...
  const payload: Partial<Industry> & { id?: string } = {
  ...
  toast.show(mode.value === 'create' ? '業界を追加しました' : '業界を更新しました')
...
<template>
  <MastersMasterShell
    title="業界マスタ"
    description="業界は直交軸で管理します（「小売×アパレル」のような複合値を作らず、会社側で複数業界を紐付ける）"
  >
    <template #actions>
    ...
    <template #filter>
      <UiSearchInput v-model="search" placeholder="業界名で検索" />
      <UiSelect v-model="statusFilter" :options="ACTIVE_FILTER_OPTIONS" aria-label="状態フィルタ" />
    </template>

    <UiSectionCard :title="`業界一覧（${total}件）`" flush>
      <UiDataTable
        :columns="columns"
        :rows="tableRows"
        clickable
        empty-title="該当する業界がありません"
        @row-click="openDetail"
      >
        <template #cell-name="{ row }">
          <span class="font-medium">{{ asIndustry(row).name }}</span>
        </template>
        <template #cell-active="{ row }">
          <UiStatusBadge :label="asIndustry(row).active ? '有効' : '無効'" :tone="asIndustry(row).active ? 'ok' : 'neutral'" dot />
        </template>
      </UiDataTable>
