/**
 * 入力サポート「AIで整形」（改修依頼 2026-08-20）。デュアルモード:
...
export type TextAssistResult =
  | { ok: true; text: string; llm: boolean }
...
export function useTextAssist() {
  ...
  async function formatText(text: string): Promise<TextAssistResult> {
    ...
    if (!text.trim()) return { ok: true, text, llm: false } // 空入力はそのまま返す（shared と同じ規約）
    if (isApi) {
      try {
        ...
        return { ok: true, text: heuristicFormatRequestText(text), llm: false }
      }
    }
    return { ok: true, text: heuristicFormatRequestText(text), llm: false }
  }
