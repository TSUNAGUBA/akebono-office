...
/** 弊社取り分（残余）の可視化・整合検証（改修 P1。設定過剰の検出） */
describe('三者按分の当社取り分（残余）と整合検証', () => {
  it('当社取り分率 = 1 − 店舗取り分 − 作家率', () => {
    // 店舗30% + 作家60% → 当社10%
    expect(residualMarginRate(0.30, 0.60)).toBeCloseTo(0.10, 10)
    // 店舗35% + 作家55% → 当社10%
    expect(residualMarginRate(0.35, 0.55)).toBeCloseTo(0.10, 10)
  })
