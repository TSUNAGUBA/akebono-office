...
| `ImprovementsRequestEditForm` | initialBody / initialTags / initialLinks / initialImages / busy / active。要望の編集フォーム共通部品（本文 textarea + 文字数カウンタ + タグ〔UiChipSelect〕+ 参考リンク・画像〔ImprovementsAttachmentEditor〕。保存は `{body,tags,links,images}` を emit・キャンセル emit。F-42-16 の本文のみ編集を全項目編集へ拡張 = 改修依頼 2026-08-19。/improvements の生要望ドロワーと ImprovementSubmit の送信直後修正で共用 = 原則3）。旧 `ImprovementsBodyEditForm`（本文のみ）は本部品へ統合し廃止 |
| `ImprovementsAttachmentEditor` | v-model:links（string[]）/ v-model:images（ImprovementRequestImage[]）/ v-model:busy（画像処理中）/ active（表示中のみ window ドロップ抑止）。要望添付（参考リンク + 画像）の編集共通部品（リンク追加/削除/編集・画像はファイル選択/D&D/貼り付けの 3 経路で縮小・上限・種別チェック = 原則3。ImprovementSubmit と ImprovementsRequestEditForm で共用。改修依頼 2026-08-19） |
