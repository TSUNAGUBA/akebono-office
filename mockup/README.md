...
8. **メディア分析**（AKEBONO 業務配下のアプリ）で、業態（セグメント）ごとに Google Analytics 連携 → アクセス指標の可視化 → AI がサイト構成・記事のインサイトと次アクションを提示 → **業務 × メディアの PDCA**（売上と流入の相関）→ **目的・質・雰囲気を指定した AI 記事生成**（過去分析からのお題提案・採用）まで体感できます（未連携の業態は設定画面から擬似 OAuth で連携）
