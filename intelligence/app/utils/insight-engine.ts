/**
 * 分析エンジン（FI-03。決定的ヒューリスティック = モック実装）。
 *
 * 【モック境界の宣言】本エンジンは共通 API の AI 推論（Vertex AI + RAG + WebSearch）で
 * 本実装するまでの暫定実装。入力データは API モードでは実データ、モックモードではシード。
...
import { normalizedIncludes } from '~/utils/search'
...
function analyzeManagement(data: EngineData, opts: EngineOptions): EngineResult {
  ...
  const salesThis = sales.filter(s => s.createdAt.slice(0, 7) === month)
