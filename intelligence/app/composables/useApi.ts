...
/** 認証ヘッダ（dev = x-dev-member-id / 本番 = Firebase ID トークン） */
...
/** コレクション名 → API マスタスラッグ（本アプリで使うマスタのみ） */
...
export function isMigratedCollection(name: string): boolean {
  return name in MIGRATED_MASTERS
}
 ...
 * useApi 管轄外のキャッシュ（AI タスク・活動ログ・通知等のドメイン別キャッシュ）はここに登録する。
 */
