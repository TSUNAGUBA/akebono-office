 ...
 * API モードの通知はサーバー（共通 API）が AKEBONO Home のパスで発行するため、
 * 本アプリ（Company）で開ける通知はパスを写像し、開けない通知（Home 側ドメイン）は null を返す
 * （呼び出し側が「Home 側の機能」である旨を案内する。壊れた遷移 = 404 を作らない）。
 *
 ...
 * - それ以外（Home の勤怠・稟議・日報等）→ null
 *
 * 注意: Home の `/reports`（日報・週報）と本アプリの `/reports`（AI 日次報告）はパスが同名で
 * 意味が異なるため、`/reports` は写像対象に含めない（API モードの `/reports` 通知は Home 側の
