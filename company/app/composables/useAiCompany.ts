/**
 * AI ネイティブカンパニー（AKEBONO Office F-08 の切り出し独立版）
 * タスク依頼 → 分解 → 承認 → 実行 → 完了報告 → 日次報告生成。
 *
 * デュアルモード（Office 版と同一の設計）:
 * - モックモード: useMockDb + 決定的分解（SoT は shared/domain/ai-tasks = API フォールバックと同一）
 ...
 *   AI 日次報告は daily_reports（author_kind='ai'）が SoT = /v1/reports/daily?scope=all の月次キャッシュから射影
...
/** AI 日次報告の月次キャッシュ（daily_reports の authorKind='ai' 射影。キー = YYYY-MM） */
const apiAiReports = ref<Record<string, DailyReport[]>>({})
...
function loadAiReports(month: string, force = false): Promise<void> {
  return apiLoadOnce(`aic:aireports:${month}`, async () => {
    const rows = await apiFetch<DailyReport[]>('/v1/reports/daily', { query: { scope: 'all', month } })
    apiAiReports.value = { ...apiAiReports.value, [month]: rows.filter(r => r.authorKind === 'ai') }
  }, force)
}
...
export function useAiCompany() {
  ...
  /** その日の AI 日次報告（API モードは daily_reports が SoT = 月次キャッシュから射影） */
  function aiReportsOn(date: string): DailyReport[] {
    if (isApi) {
      const month = date.slice(0, 7)
      void loadAiReports(month)
      return (apiAiReports.value[month] ?? []).filter(r => r.date === date)
    }
  ...
  function rollUpToParentMock(child: AiTask): void {
    ...
    // cancelled の分担は「完了待ち」に数えない（API 版と同一）
    ...
    // 全分担完了時は分担先の統合報告を集約した親の統合成果物も作る（API 版と同一）
    ...
    if (allChildrenDone) {
      ...
      notify(parent.requesterId, 'ai_report', `AI 連携完了報告: ${parent.title}`,
        `${employeeById(parent.aiEmployeeId)?.name ?? 'マネージャー'} が分担タスクの成果を統合して完了しました`, `/tasks?task=${parent.id}`)
    }
  ...
  /**
   * 全自動実行（Office 版と同一）。承認・回答・ブロック解除後、「進める」の連打なしで
   * 完了 / 依頼者への質問 / 中止 まで走り切る。分担があれば子タスク群を対象にする。
   * モックは決定的ヒューリスティックのため即座に収束する（Web 調査は API モードのみの機能）
   */
  const AUTO_RUN_MAX_STEPS = 12
  let autoRunning = false // progressTask 末尾の自動継続との相互再帰防止
  ...
  /** 分解案を承認して実行開始（proposed → 即 in_progress → 全自動実行） */
  async function approveTask(taskId: string): Promise<Result> {
    ...
    commit()
    // 承認 = ユーザーの意思表示。以降は「進める」なしで走り切る
    await autoRunMock(taskId)
    return { ok: true, id: taskId }
  }

  /**
   * 実行を 1 ステップ進める。ステップの成果物を生成して保存し、
  ...
  async function progressTask(taskId: string): Promise<Result> {
    ...
    if (question) {
      ...
      notify(task.requesterId, 'ai_report', `AI 確認依頼: ${task.title}`,
        `${emp?.name ?? 'AI社員'} から確認: ${question}（タスクボードで回答してください）`, `/tasks?task=${taskId}`)
      return { ok: true, id: taskId }
    }
    ...
    if (finished && !task.parentTaskId) {
      ...
      notify(task.requesterId, 'ai_report', `AI 完了報告: ${task.title}`,
        `${emp?.name ?? 'AI社員'} がタスクを完了しました`, `/tasks?task=${taskId}`)
    }
    // 手動「再開」でも残りのステップは自動で走り切る（autoRunMock からの呼び出し時はループ側が継続を担う）
    if (!finished && !autoRunning) await autoRunMock(taskId)
    return { ok: true, id: taskId }
  }

  /**
   * 依頼者の回答（人間のアクションが必要な箇所への応答）。
  ...
  async function blockTask(taskId: string): Promise<Result> {
    ...
    if (task.status === 'in_progress') {
      ...
      if (task.parentTaskId) {
        ...
        if (parent) {
          ...
          notify(parent.requesterId, 'ai_report', `AI 連携ブロック: ${task.title}`,
            '分担先のタスクがブロックされました。タスクボードで状況を確認してください', `/tasks?task=${parent.id}`)
        }
      }
      ...
      syncEmployeeStatus(task.aiEmployeeId)
      commit()
      // 解除後は自動で走り切る（open な質問があれば autoRunMock 側で停止）
      await autoRunMock(taskId)
      return { ok: true, id: taskId }
    } else {
  ...
  async function generateDailyReports(date: string): Promise<{ created: number; skipped: number }> {
    if (isApi) {
      try {
        ...
        // 生成された AI 日報を月次キャッシュへ反映（SoT → キャッシュ。原則6）
        if (data.created > 0) await loadAiReports(date.slice(0, 7), true)
        return data
      } catch {
  ...
  return {
    ...
    /** API モードのタスク・ログ再取得（承認後の自動実行の進捗を追うポーリングに使う） */
    reloadAi,
  }
}
