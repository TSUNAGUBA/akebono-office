...
| `ImprovementsRequestEditForm` | initialBody / initialTags / initialLinks / initialImages / busy / active。要望の編集フォーム共通部品（本文＋画像〔ImprovementsBodyImageInput〕+ タグ〔ImprovementsTagToggle = 二者択一トグル・改修依頼 2026-08-20〕+ 参考リンク〔ImprovementsLinkEditor〕。保存は `{body,tags,links,images}` を emit・キャンセル emit。F-42-16 の本文のみ編集を全項目編集へ拡張 = 改修依頼 2026-08-19。/improvements の生要望ドロワーと ImprovementSubmit の送信直後修正で共用 = 原則3）。旧 `ImprovementsBodyEditForm`（本文のみ）は本部品へ統合し廃止 |
