...
> （本実装 API と共有）。home 側の同名ファイルは再エクスポートのシムであり、**ロジック・型の変更は shared 側で行う**。
...
// 業態カードは他メニューと同じく categorizeCards で通常配置（未割当は「その他」= showOther 設定で制御。
// 最上段の固定「AKEBONO 業務」セクションと planDashboardCards は改善要望 2026-08-21 で廃止）
...
// 改善要望（F-42 → ステータス管理の再編 = 改修依頼 2026-08-21。各ページからの投稿〔初期 = 未確認〕→ 受付箱ステータスで対応方針を設定〔検討中 → 改善対応/運用対応/継続検討/対応見送り〕→ 「改善対応」のみ AI 集約 → 改修案件は 未対応 → 対応中〔担当者アサイン〕→ 対応済 の 3 状態 → 改修プロンプト出力。純ロジック SoT = shared/domain/improvement）
// 投稿は全員可（submit は管理 GET を誤発火しない）。閲覧・管理は canManageImprovements（deny-by-default + 管理者常時可 = usePermissions）。
// 集約は「改善対応（status='planned'。旧データは adoption='adopted' も対象 = 原則7）かつ未集約」の要望のみ処理・判定済み item のステータスは巻き戻さない（原則2）。API = Vertex AI → 決定的ヒューリスティック（heuristicClusterRequests）
const imp = useImprovements()  // submit（body + 対象ページ〔既定=開いているページ・全体/新設ページ可〕+ 対象箇所 targetSpot〔任意〕+ 任意添付 links〔URL 最大5〕・images〔縮小 data URI 最大4〕= 0061 + 任意タグ tags〔壁打ち brainstorm/お任せ entrust = 0065・F-42-17〕。初期ステータス = 'unconfirmed'。mock は persisted=false で容量超過を通知）/ refresh / loadRequestImages(itemId)・loadRequestImagesFor(request)（添付画像の遅延ロード）/ inboxStatusOf(r)（受付箱の表示ステータス解決 = improvementInboxStatusOf〔resolvedAt/旧 resolved → 解決済み・item 連動の 対応済み 導出・旧 adoption 読み替え = 原則7〕。mock = ローカル items / API = linkedItemStatus）/ setRequestStatus(id, base, revisitOn?, note?)（受付箱ステータスの変更 = 状態機械 IMPROVEMENT_INBOX_NEXT。operational は運用案内必須〔AKO-REQ-024〕= コメント記録 + 起票者通知・deferred は再検討日必須〔AKO-REQ-023〕。ガード = 共有 improvementInboxStatusError〔集約済み AKO-REQ-013・取消済み AKO-REQ-019・解決済み AKO-REQ-025・機械外 AKO-REQ-006〕）/ setRequestStatusBulk(ids, base)（一括変更 = planInboxStatusBulk。deferred/operational は一括対象外 = AKO-REQ-028・部分成功は done/failed = 原則4）/ resolveRequest(id, resolved)（解決済みの記録/取消 = resolvedAt オーバーレイ。運用対応 or 対応済みのみ = AKO-REQ-026・本人 or 管理）/ assessRequest(id)（AI 判定 = ナレッジベース RAG。API = Vertex → ヒューリスティック / mock = heuristicAssessRequest。verdict/根拠/参照 doc/推奨アクション = IMPROVEMENT_ASSESS_META）/ unclusterRequest(id)（集約の解除 = 改修単位から外し「改善対応（集約待ち）」へ。excludedItemIds 履歴 + 元 item へ修正メモ。ガード = improvementUnclusterError〔存在 → 未集約 → 取消済み → 解決済み AKO-REQ-025 → item 取消/決着〕）/ editRequest(id, {body, tags?, links?, images?, targetSpot?})（全項目の部分更新 = 送っていない項目は保持・editedAt 記録 = 原則9.5）/ addRequestComment・setRequestCommentArchived・commentsForRequest（要望コメント = 対応方針のやり取り・古い順）/ operationalGuidanceFor(requestId)・loadOperationalGuidance(requestId)（自分の要望の運用案内 = kind='ops' コメント〔0083 = 手動コメントとの判別キー。「運用案内: 」で始まる手動コメントは含めない = R3 監査〕。通知 OFF でもドロワーで読める。API 非管理者は requestId 指定 GET で運用案内のみ取得 = R2 監査）/ activeItems・archivedItems・unclusteredRequests・plannedUnclustered（改善対応・集約待ち）・unconfirmedRequests（未確認）・allRequests / requestsForItem / notesForItem / generate（集約 = 改善対応のみ。新規 item は 'todo'）/ setStatus(id, 'todo'|'in_progress'|'done', assigneeMemberId?)（改修案件の 3 状態機械 = IMPROVEMENT_ITEM_NEXT・旧 7 値語彙は improvementItemViewOf で正規化〔原則7〕。in_progress は担当者アサイン可・done は resolvedAt 記録 + 起票者へ「対応済み」通知・reopen で resolvedAt クリア）/ editItem（title/summary/detail + planStart/planEnd + assigneeMemberId〔'' = 解除・不正 AKO-REQ-027〕）/ setItemArchived・setRequestArchived / addNote・setNoteArchived / checkRevisitReminders（mock の継続検討リマインド = 要望 + 旧 deferred item。localStorage 日次デデュープ）/ buildPrompt(filter='todo')（未対応のみ・targetSpot 併記・【解決済み】【対応見送り】明記）。**生要望（GET /requests）は認証済み全員が閲覧可・ステータス変更・AI 判定・generate・buildPrompt・改修案件（items）は管理権限者のみ**
```
...
| `SettingsAuditLogPanel` | props なし。設定 > 監査ログタブの一覧パネル（改修 2026-08-21）。列 = 操作日時/操作ユーザー/操作対象ページ/操作対象データ/操作内容。フィルタ 5 種（期間 date 2 欄 + ユーザー/ページ/データ/内容の `UiCombobox` オートコンプリート）+ 適用中件数バッジ + 一括クリア。ページング 20 件/頁（`useListView` デュアルモード = mock: tbl('auditLogs') + filterPredicate / API: GET /v1/configs/audit-logs の f.* + limit/offset/total）。語彙の SoT = `shared/domain/audit-log`（mock の camelCase entity は normalizeAuditEntity で正規化 = 原則6）。読み取り専用（記録系 = 取消フローなし） |
...
| `ImprovementsKanban` | items（ImprovementItem[]）・reqCount(id)。ステータス別カラムのカンバン（F-42。**3 状態 = 未対応/対応中/対応済〔IMPROVEMENT_ITEM_VIEWS〕へ再編 = 改修依頼 2026-08-21。旧語彙は improvementItemViewOf で列へ正規化〔原則7〕。カードに担当者バッジ〔UserRound + 氏名〕**）。emit: open(item)・status(id,to)。許可遷移（IMPROVEMENT_ITEM_NEXT）のクイック操作〔対応中はドロワーの担当者入力へ誘導〕・横スクロール |
| `ImprovementsGantt` | items（ImprovementItem[]）。対応予定期間のガント（F-42。月次/週次/日次切替・前後送り・今スナップ）。列/バーは `shared/domain/gantt` 純関数。**ステータスフィルタ（既定 = open = 未完了〔未対応 + 対応中〕。選択肢/判定は `IMPROVEMENT_FILTER_OPTIONS`/`matchesImprovementFilter` 共有）+ バー色分け（未対応=warn/対応中=brand/対応済=muted〔退色〕= 3 状態へ再編 2026-08-21）+ 凡例 + 行ラベルに担当者名併記** emit: open(item) |
