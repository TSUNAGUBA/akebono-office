...
<template>
  ...
  >
    ...
    <p class="border-t border-line px-4 py-2 text-[11px] text-muted">
      ...
      日報・週報 = 権限表「日報・週報の参照対象」（既定 参照可）。対象は在籍メンバーのみです（無効化済みメンバーの
      登録データは本人以外の AI へ供給されません）。解決はまず参照者のレイヤ（個人 &gt; 役職 &gt; ロール）で確定し、
      同一レイヤの中では 登録者単位の設定（個人 &gt; 役職 &gt; ロール &gt; 全メンバー一括）→ 旧「AI 参照範囲」
      （すべて / 自分のみ の一括二値 = 権限表のタブで引き続き設定可）の順に参照し、どのレイヤでも決まらない
      場合に既定値を使います（例: 参照者の個人レイヤに旧二値があると、ロールレイヤの登録者単位設定より優先されます）。
      機能自体の利用拒否（ページの deny）が最優先です
    </p>
  </UiSectionCard>
</template>
