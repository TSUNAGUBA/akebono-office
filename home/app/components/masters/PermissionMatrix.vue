...
<template>
  ...
  >
    ...
    <p class="border-t border-line px-4 py-2 text-[11px] text-muted">
      ※ 管理者（ロール列の管理者・権限ロールが管理者の個人。役職経由で該当する場合も同様）の「マスタメンテナンス」「設定」「改善要望管理」への拒否、および参照対象の本人セルはロックアウト防止のため操作できません（改善要望管理は管理者を常時許可）。「全員のタイムカードの参照」の既定は管理者・人事のみ参照可です（役職列の既定表示は「参照不可」ですが、明示ルールが無い場合の実判定は本人のロール既定に従います）。タブ行は ✓ = 利用可 / × = 利用不可（管理者限定タブ〔稟議の全件・経路設定、シフトの調整・募集期間〕はロールガードが基底のため、利用可にしても非管理者へは開放されません。タブの制御は画面上の表示・遷移のみで、データ面の権限〔機能・項目・参照対象のルール〕は変わりません）。AI 参照範囲行は ✓ = すべて / × = 自分のみ（登録者単位の細かい設定は「AIの参照範囲」タブで行えます）、参照対象行は ✓ = 参照可 / × = 参照不可 を表します。マスタ項目の表示制御はページの利用可否とは独立に、アプリ全体のマスタ応答へ適用されます（ドキュメントのみページ行が項目の一括既定を兼ねます）。項目の「更新」行は参照＞更新の階層です: 項目の参照が不可の場合は更新も不可となり、更新セルは操作できません（参照不可のレイヤへ更新ルールを置きたい場合〔他レイヤの参照許可者向け〕はルール一覧モードから設定できます）。「週報」「月報」は「日報」から独立した機能キーです: 週報・月報のルールが 1 件も無い間は従来の「日報」（旧・日報・週報）の設定〔旧タブキー weekly-*/monthly-* 含む〕を実判定が継承し、週報・月報のルールを設定した時点でその機能は独立制御になります（本表の薄色セルはアプリ既定値を表示しています）。個々のルールの無効化・復元の履歴はルール一覧モードで確認できます
    </p>
  </UiSectionCard>
</template>
