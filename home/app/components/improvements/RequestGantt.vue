 ...
 *
 * 設計判断（2026-08-21）: バー色・絞り込みのステータス軸は要望カンバン（RequestKanban）と同じく
 * **受付箱ステータス（IMPROVEMENT_INBOX_STATUSES。集約済みは item 連動の導出）**に統一する。
 * 絞り込みは IMPROVEMENT_INBOX_FILTER_OPTIONS / matchesInboxFilter を共用（原則3/5/6）。
...
import {
  IMPROVEMENT_INBOX_FILTER_OPTIONS, IMPROVEMENT_INBOX_STATUS_META, IMPROVEMENT_INBOX_STATUSES,
  type ImprovementInboxFilter, type ImprovementInboxStatus, type ImprovementRequest,
  matchesInboxFilter,
...
/** 要望の表示ステータス（親から渡された解決関数 = RequestKanban と同一写像） */
function statusOf(r: ImprovementRequest): ImprovementInboxStatus {
  return props.statusOf(r)
}

/** ステータス絞り込み（既定 = すべて。選択肢・判定は受付箱共通 = 原則3） */
const statusFilter = ref<ImprovementInboxFilter>('all')

/** バー色（受付箱ステータス別・凡例と一致 = 改修依頼 2026-08-21 のステータス定義に合わせた色分け） */
const BAR_CLASS: Record<ImprovementInboxStatus, string> = {
  unconfirmed: 'bg-warn', // 未確認 = 確認待ち（アンバー）
  reviewing: 'bg-info', // 検討中
  planned: 'bg-brand', // 改善対応 = 改修予定（ブランド青）
  operational: 'bg-ok', // 運用対応 = 運用でカバー（決着）
  deferred: 'bg-serious', // 継続検討 = 再検討待ち
  dismissed: 'bg-crit', // 対応見送り（レッド）
  addressed: 'bg-ok', // 対応済み = 改修完了・起票者確認待ち（グリーン）
...
/** 絞り込み後・投稿日昇順（判定は受付箱共通の matchesInboxFilter = 原則3） */
const filtered = computed(() =>
  props.requests
    .filter(r => matchesInboxFilter(statusOf(r), statusFilter.value))
    .slice()
...
const legendStatuses = computed<ImprovementInboxStatus[]>(() =>
  IMPROVEMENT_INBOX_STATUSES.filter(s => visible.value.some(r => statusOf(r) === s)))
...
function setStatusFilter(v: string): void { statusFilter.value = v as ImprovementInboxFilter }
...
<template>
  <div class="grid gap-3">
    ...
    <!-- ステータスで絞り込み（受付箱共通の選択肢 = カンバンの列と同じ軸） -->
    <UiChipTabs
      :model-value="statusFilter"
      :options="IMPROVEMENT_INBOX_FILTER_OPTIONS"
    ...
    <div class="flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
      ...
      <div v-if="legendStatuses.length" class="flex flex-wrap items-center gap-x-3 gap-y-1">
        <span v-for="s in legendStatuses" :key="s" class="flex items-center gap-1 text-[11px] text-muted">
          ...
          {{ IMPROVEMENT_INBOX_STATUS_META[s].label }}
        </span>
      </div>
    </div>
        ...
        >
          ...
          <div class="relative flex flex-1">
            ...
            <button
              ...
              :title="`${bodyLine(r)}（${fmtDate(r.createdAt)}・${IMPROVEMENT_INBOX_STATUS_META[statusOf(r)].label}）`"
              @click="emit('open', r)"
            >
