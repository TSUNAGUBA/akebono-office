...
/**
 * ぽいぽいポスト（改善のタネ）の投稿導線（改修依頼 2026-08-20 → 名称・項目順変更 2026-08-21）。
 * アプリヘッダーの「ぽいぽいポスト」ボタンを、/poipoi ページへの遷移ではなく、この場で登録フォーム（モーダル）を開く導線に変える。
 * 項目順は「内容（本文）→ 任意項目（プロジェクト/顧客/業務種別）」（改善要望 2026-08-21）。
...
<template>
  ...
  <button
    ...
    aria-label="ぽいぽいポストを投げ込む"
    @click="openModal"
  >
    <StickyNote class="h-4 w-4" aria-hidden="true" />
    <span class="hidden lg:inline">ぽいぽいポスト</span>
  </button>
  ...
  <UiModal :open="open" :title="sent ? '投げ込みました' : 'タネを投げ込む'" width="560px" @close="open = false">
    <div v-if="!sent" class="grid gap-3">
      ...
      <!-- 項目順は「内容（本文）→ 任意項目」（改善要望 2026-08-21。/poipoi の投稿フォームと同順） -->
      ...
      <!-- 任意項目（/poipoi と同じ紐付け。プロジェクト選択で顧客を補完） -->
      <UiFormField label="任意項目" hint="プロジェクト・顧客・業務種別を紐付けできます">
      ...
      <p class="text-[11px] text-muted">本文はマークダウン記法に対応（表示時に描画されます）。詳しい登録（ファイル取込・プレビュー等）はぽいぽいポストページで行えます。</p>
    </div>
    ...
    <div v-else class="grid gap-3">
      ...
      <p class="text-[12px] text-muted">誤って投げ込んだ場合は「取り消す」で取り消せます（ぽいぽいポストページからも復元できます）。</p>
    </div>
