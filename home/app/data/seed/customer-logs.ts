/**
 * 顧客活動（旧「顧客ログ」= 改修依頼 2026-08-18 で改称）のシード（デモ用・決定的・今日基準の相対日付）。
 * 記録者（member）・会社（company）・担当者（contact）は core シードの id を参照する。
 * 一覧は全メンバーの記録を全員が閲覧できるため（改修依頼 2026-08-18）、記録者は m-03/m-04/m-05 に分散させる。
 * 活動目的（旧「属性タグ」）・開始/終了時刻・自社担当者・担当者メモは 2026-07-31 の項目拡張、
 * 活動手段（method）は 2026-08-18 の改修依頼で追加
 * （旧データの body は担当者メモとして継続 = 原則7 の互換方針をシードにも反映）。
 * 議事録メモ（minutesMemo）は 2026-08-03 のオペレーター指示で廃止。
 */
