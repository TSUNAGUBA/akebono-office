...
#
# AI ネイティブ開発方法論テンプレート（ai-native-dev-operation-template）由来の
# 共通実行ラッパー。GitHub Actions（.github/workflows/deploy.yml）の各テストゲート
...
#       bash scripts/run-test-stage.sh "結合テスト" integration-test
#
#   引数:
#     $1 = ステージ表示名（例: 単体テスト）
#     $2 = ログファイル名（拡張子なし。例: unit-test）
#   環境変数:
#     STAGE_CMD          = 実行するコマンド（ワークフロー側で注入）
...
if [ -z "${STAGE_CMD:-}" ]; then
  echo "::error::${STAGE_NAME} の実行コマンド（STAGE_CMD）が設定されていません。ワークフローの設定を確認してください。"
  exit 1
fi
